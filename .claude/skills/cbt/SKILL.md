...
**Mobile nav = the inline nav, wrapping (HO 156 follow-up).** At 6 nav items the inline `HeaderNav` / `.home-header-nav` wrap cleanly into two rows on a 375px phone, so the inline nav *is* the mobile nav at every width — there is no hamburger. `.home-header-nav` already carries `flex-wrap: wrap`; nothing hides it on mobile.
...
| HeaderNav / home-nav | 16 / 14–16px | same | same | Inline nav **wraps** at all widths (drawer dormant — see above) |
