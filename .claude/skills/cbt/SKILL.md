...
- `/races` — competitive-races surface (HOs 62/71/72). Renders the **`races` electoral GROUP_TABS group** (Races · Primaries, HO 173 — no longer the Members sub-strip). Server page; `getRacesIndex` (+ `getUpcomingPrimaries` / `getPastPrimaries` for each race's primary date), rendered as `race-row`s. No URL params. Links into the `/race/[id]` hubs; the `ABS(rating_score) <= 1` competitive cut and the per-source `RatingChip` rendering match the "Race ratings" section.
