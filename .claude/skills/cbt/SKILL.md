...
- `/` — dashboard (HO 131 redesign, supersedes HO 126's 2x2 grid). Top-of-page `HomeHeader` (replaces the old `HeaderBar variant="dashboard"` chrome) stacks in order: (1) `Congress Terminal:\>` prompt on its own line (26px monospace, `--text-primary` + `:\>` in `--accent-amber`); (2) cron-generated LEAD prose with viewport-tiered line-clamp (3 lines ≥1920px, 2 lines 1199–1919, 1 line <1199 — font-size stays constant, the visible line count is what scales); (3) single-line `· LAST SYNC HH:MM MT · N BILLS TRACKED` meta (11px `--text-muted`); (4) `--border-soft` separator, then a 12-link nav row (every link carries HO 131 tooltip copy via `title=` + `aria-label`, source-of-truth `NAV_ITEMS` in `HeaderBar.tsx`). The previous standalone `Lead · Last updated …` chip was dropped per HO 131 D. Body is a 3×2 `.home-grid` with `height: calc(100vh - var(--home-header-height) - 24px)` and `overflow: hidden` quadrants — the cell decides how many rows are visible, no hardcoded N. Row 1: BREAKING (3-row 72h feed) · ACTIVITY (`ActivityTicker`, capped by overflow) · **COLOR KEY** (read-only legend block). Row 2: STAGE DISTRIBUTION (6 bars) · TOPIC DISTRIBUTION (24-row bar list with `overflow-y: auto` inside the quadrant — HO 132 swaps in a bubble) · **TOP STALLS** (5 rows). Right-column reads top→bottom as `legend then deeper read`. Primary target 2560×1440; scales fluidly to 1920×1080 without scroll. At 1440×900 the grid still renders 3×2 and may scroll modestly per spec. Below 1200px viewport the grid relaxes to 2×3 with auto-height; below 700px collapses to single-column stack. Accepts `?stage=<stage>` and `?topics=<topic>` query params for click-to-filter — single value each, validated against the Stage and Topic enums, invalid values silently dropped (no `q`/`sort`/`sponsor`/`cluster` on `/`). Selected bar/row renders at full opacity, others at 0.4. `ActiveFilterStrip` renders between the header and the grid when any filter is active, with `× CLEAR` (→ `/`) and `VIEW IN /FEED →` (→ `/feed?stage=…&topics=…`) links. Because `await searchParams` opts the route into dynamic rendering, `/` is no longer statically prerendered — the `bills`-tagged query cache still applies at the query layer. The `DashboardLead` standalone component was deleted in HO 131; `getDashboardLead()` is now called directly from `HomeHeader`.
...
`--home-header-height: 180px` is the single fixed offset used by `.home-grid`'s `calc(100vh - var(--home-header-height) - …)` no-scroll math. Sized for the widest case (≥1920px viewport with `.home-header-lead` line-clamp: 3). At narrower viewports the lead clamps to 2 then 1 lines, so the actual header shrinks and the grid gains a small slack — no visual harm. Bump only if the header gains a new row (additional meta line, nav wraps to two rows at desktop widths).
