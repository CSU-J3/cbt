...
- `/` — dashboard (HO 131 redesign, supersedes HO 126's 2x2 grid). Top-of-page `HomeHeader` (replaces the old `HeaderBar variant="dashboard"` chrome): `Congress Terminal:\>` prompt (26px monospace, `--text-primary` + `:\>` in `--accent-amber`), single-line `· LAST SYNC HH:MM MT · N BILLS TRACKED` meta (11px dim), then the cron-generated LEAD prose folded into the header (line-clamp-3, max 1200px wide; the previous standalone `Lead · Last updated …` chip was dropped per HO 131 D), then a 12-link nav row below a soft border (every link carries the HO 131 tooltip copy via `title=` + `aria-label`, source-of-truth `NAV_ITEMS` in `HeaderBar.tsx`). Body is a 3×2 `.home-grid` with `height: calc(100vh - var(--home-header-height) - 24px)` and `overflow: hidden` quadrants — the cell decides how many rows are visible, no hardcoded N. Row 1 quadrants: BREAKING (3-row 72h feed) · ACTIVITY (`ActivityTicker`, capped by overflow) · TOP STALLS (5 rows). Row 2 quadrants: STAGE DISTRIBUTION (6 bars) · TOPIC DISTRIBUTION (24-row bar list with `overflow-y: auto` inside the quadrant — HO 132 swaps in a bubble) · COLOR KEY (HO 131 read-only `ColorKey` block listing the 6 stage tokens, 8 topic groups, 3 party tokens, and the amber accent). Primary target 2560×1440; scales fluidly to 1920×1080 without scroll. At 1440×900 the grid still renders 3×2 and may scroll modestly per spec. Below 1200px viewport the grid relaxes to 2×3 with auto-height; below 700px collapses to single-column stack. Accepts `?stage=<stage>` and `?topics=<topic>` query params for click-to-filter — single value each, validated against the Stage and Topic enums, invalid values silently dropped (no `q`/`sort`/`sponsor`/`cluster` on `/`). Selected bar/row renders at full opacity, others at 0.4. `ActiveFilterStrip` renders between the header and the grid when any filter is active, with `× CLEAR` (→ `/`) and `VIEW IN /FEED →` (→ `/feed?stage=…&topics=…`) links. Because `await searchParams` opts the route into dynamic rendering, `/` is no longer statically prerendered — the `bills`-tagged query cache still applies at the query layer. The `DashboardLead` standalone component was deleted in HO 131; `getDashboardLead()` is now called directly from `HomeHeader`.
...
`--home-header-height: 220px` is the single fixed offset used by `.home-grid`'s `calc(100vh - var(--home-header-height) - …)` no-scroll math. Bump if `HomeHeader`'s rendered height shifts (e.g. title font-size change, additional meta line, nav wraps to a new row at desktop widths).
