...
- `/races` — competitive-races surface, **map-first as of HO 210** (HOs 62/71/72/210). Renders the **`races` electoral GROUP_TABS group** (Races · Primaries, HO 173). Server page. **A literal `geoAlbersUsa` US choropleth (`CartogramShell`, client) is the DEFAULT view; the existing spectrum-bar `race-row` LIST is one MAP/LIST toggle away (kept as-is, NOT rebuilt).** A tile-grid cartogram built earlier in HO 210 was **rejected in design** ("squares are lifeless, don't read as 'my state' — geographic legibility won") and replaced by the literal map — **do NOT re-optimize back to squares.** `/primaries` mounts the same `CartogramShell` with the PRIMARIES variant (see that entry).
  ...
  - **Pinned card (BASIC + null-safe, HO 210 Pass 2; `RaceMapCard` / `PrimaryMapCard`):** RACES = **Senate-first** click-to-expand accordion (single-open) — collapsed: incumbent face (`incumbent_depiction_url`, `onError`→initials; **137/137 rated incumbents have a photo and a resolved name — a blank row would be a bug, not data**) · `[party] SEAT · name` · consensus rating; expanded: incumbent (→ member page) + challenger rows **or** a "Challenger field not yet available" placeholder (`race_candidates` is empty for **133/137** races today) + Race hub link. **No margin bar** (no 2024-margin column exists in any table). PRIMARIES = VOTED → the HO 207 `ShareBar` (exported from `PrimaryRow`, props-only `{cands,tint}`, reused as-is) / UNVOTED → `SCHEDULED · <full date>` + incumbent-first field list. **Card boundary:** everything richer — Kalshi, cash, 2024 margin, rater spread, trend sparkline, news-to-seat, BREAKING strip — is the SEPARATE **rich-card HO**; `RaceMapCard` leaves documented null-safe insertion points.
  - **Data plumbing:** cells are built by `lib/cartogram-data.ts` (`buildRacesCartogram` / `buildPrimariesCartogram`, pure — no DB). `getRacesIndex` projects `incumbent_depiction_url` (members JOIN) + consensus rating onto `RaceIndexRow`; `getRaceCandidatesForCycle(cycle)` returns a **flat array** (grouped in the builder — a `Map` would round-trip to `{}` through `unstable_cache`'s JSON serialization); the raw `PrimaryWithCandidates` rides on each primaries contest for the ShareBar.
