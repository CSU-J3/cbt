...
- `/primaries` — 2026 primary + runoff calendar (HOs 91-96/107/120; redesigned HO 203). Server page; `GroupTabs` (the **`races` electoral group** as of HO 173 — sub-strip Races · Primaries; lights the Races top-nav tab, not Members); the per-page `TerminalPrompt` masthead was removed in HO 185 (see `/reports`). **No filter params** — it loads `getUpcomingPrimaries(300)` + `getPastPrimaries(300)`, groups by date (today first; past collapsed in a `<details>`); there's no `?chamber`/`?party`/`?page` (the entry that claimed those + a `PrimaryCalendarRow` was stale — neither exists). **HO 203 chrome:** passes `cursorAtEnd` + `liftSyncContrast` (matching Members/Races) — breadcrumb `Congress Terminal:\119TH\Races\Primaries>`, cursor trailing the full sync, run `--text-muted` + count `--text-secondary`. No `search bills…` band to drop (bare `HeaderBar`, no `feedFilters`), so no `pageOwnsControls` migration. **HO 203 row (`PrimaryRow`, a client island):** the old bare "N candidates" count is replaced by a **candidate-field list** — incumbent-first, top-3 last names + "+N other(s)", the **seat incumbent ★-flagged**; empty rosters (~50 of 447 upcoming) show muted "roster pending". **Seat-incumbent ★ semantics:** the ★ marks only the actual officeholder of THAT seat, via a `seat_incumbent_bioguide` subquery in `PRIMARY_SELECT` (current House officeholder matched by `state`+`district`, `is_current`), falling back to the raw `incumbent` flag where unresolved (at-large/senate/open — ≤1 there). It is **not** the raw `primary_candidates.incumbent` flag, which means "is a sitting member of Congress" and legitimately flags >1 candidate in top-two/redraw contests (CA-40: Calvert+Kim; TX-18: Menefee+Green) — that flag is unchanged and still drives the member-page links (Calvert links to his hub, just isn't ★'d in CA-40). Chosen over `races.incumbent_bioguide_id` because `primaries.race_id` is dead (3/907) and that field has cycle gaps + excludes at-large. **HO 148 click-to-expand (added; was none), single-open** via a shared `PrimaryExpandProvider` context (client-side — wraps both the upcoming groups and the past `<details>` so it survives the native collapse): full candidate field, `★ INC` flag, party, status, `/members/[bioguideId]` links for sitting members. **Share bar — UI still pending, data now UNBLOCKED.** A candidate-field share bar was specced (HO 203) but deferred because `primary_candidates.vote_pct` was 100% NULL. **HO 206 populated `vote_pct` from Ballotpedia results** (see `backfill:primary-results`), so the bar's data prerequisite is met — it should use a **results** share (not polling; no polling model exists) and keep a no-data fallback for the un-voted ~half (NULL `vote_pct`). The bar UI itself is a separate follow-up. Reads `primaries` + `primary_candidates` (see "Runoff tracking" + the primaries cron notes).
