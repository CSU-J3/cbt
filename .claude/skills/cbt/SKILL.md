...
Bloomberg Terminal aesthetic. Dark monospace, dense rows, color-coded stages and topics. No light mode. Tailwind v4 only (no shadcn / no other libraries). Server components by default. Client islands are now numerous (28 `"use client"` components as of HO 156) — filter/URL chips (`StageFilter`, `TopicFilter`, `ChamberToggle`, `ChamberFilterChips`, `PartyFilter`, `StateFilter`, `SegmentedToggle`, `NewsFilters`, `SortDropdown`, `CeremonialToggle`, `Pagination`, `SearchBox`, `SearchTabs`), the feed accordion (`BillRowList` / `BillRow` / `BillExpandedPanel`), interactive charts (`DashboardBubbleChart`, `MemberProductivityScatter`, `PatternBubbleSVG`, `PatternDrilldownPanel`), chrome (`MarketsTape` / `MarketsTapeClient`, `ActivityTabs`, `GroupTabs`, `ActiveFilterStrip`, `Tooltip`, `MobileNavDrawer` — dormant), and `WatchlistToggle`. See `### Server / client split` for the canonical set.
...
- `/primaries` — 2026 primary + runoff calendar (HOs 91-96/107/120). Server page; `GroupTabs` + `TerminalPrompt`. Backed by `getUpcomingPrimaries` / `getPastPrimaries` / `getPrimaryStats`; URL params `?chamber` / `?party` (sanitized via `sanitizePrimaryChamber` / `sanitizePrimaryParty`) + `?page` (`PAGE_SIZE = 30`). Rows render via `PrimaryCalendarRow`. Reads `primaries` + `primary_candidates` (see "Runoff tracking" + the primaries cron notes).
- `/races` — competitive-races surface (HOs 62/71/72). Server page; `getCompetitiveRaces` + `getRaceStats` into `CompetitiveRacesView`. No URL params. Links into the `/race/[id]` hubs; the `ABS(rating_score) <= 1` competitive cut and the per-source `RatingChip` rendering match the "Race ratings" section.
- `/trends` — corpus trend charts. Server page; `getBillsTimeSeries` (rendered by the `BillsTimeSeries` SVG chart) + `getTopicTrends`. SVG conventions are covered under "### Chart idiom".
...
- The full `"use client"` set is 28 components as of HO 156 (grep `"use client"` under `components/` to refresh — this list drifted to just two for several handoffs before the HO 160 sweep): filter/URL chips `StageFilter` `TopicFilter` `ChamberToggle` `ChamberFilterChips` `PartyFilter` `StateFilter` `SegmentedToggle` `NewsFilters` `SortDropdown` `CeremonialToggle` `Pagination` `SearchBox` `SearchTabs`; feed accordion `BillRowList` `BillRow` `BillExpandedPanel`; interactive charts `DashboardBubbleChart` `MemberProductivityScatter` `PatternBubbleSVG` `PatternDrilldownPanel`; chrome `MarketsTape` `MarketsTapeClient` `ActivityTabs` `GroupTabs` `ActiveFilterStrip` `Tooltip` `MobileNavDrawer` (dormant); and `WatchlistToggle`. Most exist only to push/replace URL state via the router; `WatchlistToggle` is the lone POST.
