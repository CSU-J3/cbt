...
- `/` — dashboard (HO 131 foundation, evolved through HO 133/134/132). **`HomeHeader`** stacks four blocks: (1) prompt + LEAD on a single baseline row — the `Congress Terminal:\>` prompt (26px mono, `--text-primary` with `:\>` in `--accent-amber`) is a fixed-width inline element on the left; the LEAD paragraph wraps to its own column on the right (14px, `--text-primary`, viewport-tiered line-clamp 3/2/1 ≥1920/1199–1919/<1199); baseline alignment puts the prompt's text baseline on the lead's first line (HO 134); (2) `· LAST SYNC HH:MM MT · N BILLS TRACKED` meta (11px `--text-muted`); (3) full-width nav row with all 5 `NAV_ITEMS` from `HeaderBar.tsx` (each link carries `title=` + `aria-label` tooltip copy, 30px column gap, `flex-start` alignment). The first entry is `⌂ DASHBOARD` (HO 140), routing to `/` with tooltip `Dashboard summary`; it's amber-highlighted on the home header since this header only renders on `/`, and `pathToNavKey('/')` returns `"dashboard"` so the same tab is also active in `HeaderBar`'s `HeaderNav` on sub-routes. Additive to the existing title-block click-to-home affordance — both work. (4) **`ColorKeyStrip`** — persistent five-section color legend (STAGES / TOPICS / PARTIES / BILL TYPES / ACCENT) rendered below the nav with a dashed top border, each section a `color-key-strip-cluster` inline-flex that keeps its label tight against its chips while the outer strip wraps gracefully between sections; sourced from `lib/color-key.ts` `Swatch[]` constants (STAGES / TOPIC_GROUPS / PARTIES / BILL_TYPES) so the strip and any future consumer share one table; STAGES chips render with arrow prefixes (`▸ INTRO`, `▸▸ FLOOR`, `▸▸▸▸ PRESIDENT`, `✓ ENACTED`), other sections render with `color-key-strip-dot` chips (HO 134 amendment). Below the header sits a 2×2 `.home-grid` with `grid-template-rows: auto minmax(0, 1fr)` (row 1 sizes to content, row 2 claims the remaining viewport) and `height: calc(100vh - var(--home-header-height) - 24px)`. **Row 1 left** — BREAKING single-view (no tab strip, single `home-quadrant-label` "Breaking · Last 72h (N)" with `home-quadrant-label-count` count chip, then `BreakingNewsBlock` capped at 5 rows + `.home-expander` link to `/news`). **Row 1 right** — `ActivityTabs` client island with local `useState`, two tabs: ACTIVITY (default, badge = total stage changes in 7d) and TOP STALLS (badge = 5 cap); reuses `.search-tabs.breaking-stalls-tabs` styling; lifts the prior TOP STALLS quadrant into a sibling tab; `TopStalls` footer migrated to `.home-expander` for visual consistency (HO 132 step 1). **Row 2** — two equal-width cells holding `DashboardBubbleChart` instances (HO 132 steps 2–3): left = STAGE DISTRIBUTION (6 bubbles, one per `FUNNEL_STAGES` entry, fed by `getStageDistribution`), right = TOPIC DISTRIBUTION (up to 24 bubbles, fed by `getTopicDistribution`). The chart is a "use client" island using `d3-hierarchy::pack` with `value = Math.max(count, 1)` for sqrt-faithful area encoding; each `BubbleDatum` carries `id/label/count/color/tooltip?/href?`; pre-computing href on the datum avoids passing a callback across the RSC boundary. Click triggers `router.push(href, { scroll: false })` (soft nav). Zero-count bubbles (e.g. PRESIDENT today) render at min radius 12px, `fill-opacity: 0.35`, no `<title>`, no click — `dashboard-bubble.is-static`. Label tiers in `labelFor`: `r<16` → no inline text (relies on native tooltip), `r<28` → label only, `r≥28` → label + count stacked. Hover gets `--accent-amber` stroke; focus-visible gets `--accent-amber-bright` stroke. SVG fills its cell via `width:100% height:100%` so taller viewports get bigger bubbles rather than a dead-space gap. Bubble click behavior (HO 132.1): plain click does NOT navigate — `<a href={feedHref}>` wraps each clickable `<g>` so cmd/ctrl/shift/alt-click and middle/aux-click pass through to `/feed?stage=…&topics=…` in a new tab (browser-native); plain primary click does `e.preventDefault()` + `router.push('/?…', { scroll: false })` toggling that dimension's param while preserving the other. Re-clicking the selected bubble drops that param. Each chart takes a `paramKey: "stage" | "topics"` so the stage chart writes `?stage=` and the topic chart writes `?topics=`. Selected bubble (URL value matches bubble id) gets `.dashboard-bubble.selected` with `--accent-amber-bright` stroke 2.5 + full opacity. **`DashboardBillsDrawer`** mounts in `app/page.tsx` and stays mounted at all times (toggling `data-open="true|false"` on the panel + backdrop) so both the 200ms ease-out open and 150ms ease-in close CSS transitions get a chance to play; opens whenever `?stage=` or `?topics=` is in the URL. The drawer is a thin client island (`useSearchParams`, `useRouter`, ESC handler) — it receives the bills list as `children` from page.tsx, which conditionally renders a server-component `<DashboardDrawerBody>` only when at least one param is set. Body queries via `getDashboardDrawerBills` (thin wrapper over `getFeedBills` capped at 10 rows, single-topic shape `{stage?, topic?}`), renders per-dimension chips with × clear links that preserve the OTHER dimension's param (`/?topics=…` drops stage, `/?stage=…` drops topic), total count, View-in-/feed link, compact `BillRow` list, and a trailing `.home-expander` "View all N in /feed" footer. Empty-filter intersection renders "No bills match this filter." with the chips still visible. Dismiss (X button / backdrop click / ESC) calls `router.push('/', { scroll: false })` — clears BOTH params in one go, which also clears the dashboard's existing click-to-filter narrowing; by design. Trade-off: server re-renders mid-close-animation drop `children` to null, so the panel slides out with an empty body for ~150ms. Drawer width 440px desktop, 100vw at ≤700px. Backdrop scrim `rgba(0,0,0,0.5)`, z-index 50/51. The route accepts `?stage=<stage>` and `?topics=<topic>` for click-to-filter (single value each, validated by `sanitizeStage`/`sanitizeTopic`, invalid values silently dropped; no `q`/`sort`/`sponsor`/`cluster` on `/`). `ActiveFilterStrip` renders between the header and the grid when a filter is active, with `× CLEAR` (→ `/`) and `VIEW IN /FEED →` links. `await searchParams` opts the route into dynamic rendering — `bills`-tagged query cache still applies at the query layer. `--home-header-height` is 220px to absorb the multi-line color-key strip; the no-scroll grid math is built off that var. The deleted predecessors: `ColorKey` (HO 134 dropped the full panel — the tab inside `BreakingStallsTabs` is gone too, that file deleted), `StageFunnel` + `TopicDistribution` (HO 132 step 3 — both retired in favor of the bubble pair, along with all `.funnel-*` / `.topic-dist-*` CSS), `BreakingStallsTabs` (HO 132 step 1 — single-tab BREAKING + standalone ACTIVITY/STALLS no longer warrants the client island).
...
- `/members/[bioguideId]` — member hub (handoff 60, +61, +62, +77, +80). Photo (depiction_url, plain `<img>` with initials fallback), name, party + state + district chip, born year, next-election chip (links to `/race/<id>` when the member has a non-null `next_election_year` — handoff 62), and the top-2 caucus badges by priority appended to the meta line (handoff 61), and — for Senate Democrats only — the USCPR Palestine scorecard letter grade as a `PalestineBadge` chip at the end of the meta line (handoff 138). Grade-to-color is three tiers: A/B render in `--text-secondary` (covered, not flagged), C in `--accent-amber`, D/F in `--vote-nay` (alarm; rose rather than `--party-republican` to avoid cross-wiring with the party dot on a Dem header). Source attribution lives in the tooltip (`USCPR Palestine scorecard: F (rank #3 of 47)`), never on the visible chip. Hub header only — no row-list, no BillRow sponsor expansion: row-list badging would mean ~47 of 535 members get badged and the asymmetry reads as data-missing instead of editorial scope. Absence of badge is the correct signal (no "—"/"N/A" placeholder). The full PALESTINE SCORECARD section from HO 90 stays put further down the page; the chip just elevates the grade to glance level. Stat block (bills sponsored, enacted with %, avg cosponsor count). Affiliations row below stats renders every caucus badge by priority; absent entirely for unaffiliated members (no "Coming soon"). Top 10 sponsored bills via reused `BillRow` (no expand, no daysSinceMode). `[View all N bills →]` links to `/feed?sponsor=<bioguide_id>`. **Voting record section** (handoffs 77 + 80): a `MemberVoteStats` summary line (`House votes · N total · yea% · nay% · missed%` — chamber label flips for Senate members) plus the most-recent votes rendered by `MemberVoteRow`. `getMemberVotes` has no chamber filter; a senator's page shows their Senate votes and a House rep's page shows House votes purely because they have no `member_votes` rows in the other chamber. Empty state: `No {chamber} votes recorded for this member yet.` Stock-trades block sits below the voting section. Unknown bioguide_id renders a friendly empty state, not Next's `notFound()`. Reads from `members` + `affiliations` + `votes`/`member_votes` + `stock_trades`; refresh via `npm run sync:members`, `npm run seed:affiliations`, `npm run sync:votes`, `npm run sync:senate-votes`.
...
- `/news` — breaking news + per-bill mention filter (HO 69 + 130). Default view shows the last 24h of `news_mentions` joined to `bills`, rendered via `NewsRow` with `showFullHeadline`. `HeaderBar` runs in `pageTitle="Breaking news · Last 24h"` + `pageCount` mode. `?bill=<congress>-<type>-<number>` (validated by `sanitizeBillId` — regex `^\d+-[a-z]+-\d+$`, lowercased) filters the list to a single bill via `getNewsForBill(billId, 50)`, ignoring the 24h window so older mentions stay reachable. Filtered view swaps the page title to `News mentions · <BILL ID>` and shows a `Filtered to <BILL ID>` chip with a `[ Clear filter → all recent news ]` link back to `/news`. Invalid `?bill=` value falls back silently to the unfiltered view. Empty states: `No news mentions in the last 24 hours.` (unfiltered) or `No news mentions yet for <BILL ID>.` (filtered). The HO 130 media-attention chip on every `BillRow` links here with `?bill=<id>`.
