...
Four cron jobs on Vercel Hobby, all daily, staggered by hour (Hobby caps cron at once-per-day; each invocation is a 60s-max function):

- `/api/sync` — 09:00 UTC — bills: sync + summarize (12/run) + dashboard lead + news + trades + Monday weekly report.
...
The primaries cron does **not** scrape a region per tick. The corpus is ~430 scrape units (1 Senate pass + 429 House districts); at ~1.5-2s per district — the Ballotpedia politeness sleep dominates — any whole region blows the 60s ceiling (West measured 153s warm-cache, ~200s cold). Instead `runPrimariesCronTick` (`lib/primaries-sync.ts`) walks a persistent cursor stored in `dashboard_state` under key `primaries_cron_cursor`: one tick runs the whole Senate pass, or `CRON_HOUSE_SLICE` (20) House districts, then advances the cursor and wraps at the end. Full-corpus refresh takes ~23 days. The cursor is written only after a tick's scrape succeeds, so a timed-out or failed tick re-runs the same slice next day (all upserts are idempotent). Lower `CRON_HOUSE_SLICE` if the route's `elapsedMs` log trends toward 60000.
...
- **Ballotpedia nonpartisan House primaries (HO 96).** Top-two (CA, WA) and top-four (AK) U.S. House primaries render identically on Ballotpedia — a single `<div class="race_header nonpartisan">` votebox, not split D/R sections. `parseCandidatesPage` (`lib/primary-candidates-scrape.ts`) already handles all three: a `nonpartisan`-classed votebox routes to the `"open"` contest, with each candidate's party read per-row from the `image-candidate-thumbnail-wrapper` class. There is no separate top-two/top-four parser and none is needed. `syncHouseDistricts` (`lib/primaries-sync.ts`) picks the contest set per district from `NONPARTISAN_HOUSE_STATES` — `["open"]` for CA/WA/AK, `["D","R"]` elsewhere — and stores nonpartisan rosters under a `house-{ST}-{DD}-2026-open` primary id with `party='open'`, the same convention the Senate side uses.
