...
- Size tiers (post handoff 20 readability bump; previous values shown in parentheses):
  ...
  - **14px** (was 12px) — body content: bill IDs/titles in rows, sponsor names, stage indicators, expanded summaries (`text-sm leading-relaxed`).
...
- **`HeaderBar`** — every other page. **HO 185** replaced the `CBT // 119th Congress` title with the same breadcrumb path (`Congress Terminal:\119TH\<Section>[\<Detail>]>_`, via `BreadcrumbMasthead` + `breadcrumbSegments(basePath, …)`), mounts the shared `DualMarketsTape`, and **moved the nav onto its own full-width row** (`.header-nav-row`) so it no longer competes with the path. The masthead row now holds path + bills-count/last-updated line + `SearchBox` + `CeremonialToggle` (when `feedFilters` present). The dead `variant="dashboard"` branch was deleted (zero callers).
...
| HeaderBar breadcrumb path (`Congress Terminal:\…>_`, HO 185) | 18px | 18px | 18px | Holds; a deep path wraps `<700` (labels stay intact, caret rides the end) |
...
Markets ticker tape(s). **HO 178: the dashboard mounts TWO counter-scrolling tapes** (`HomeHeader`, in a `.markets-tape-block` between the masthead and the nav) — **equities** (9: SPX, NDQ, DOW, ITA, XLK, XLV, XLF, XLE, XLI) scrolling one way and **commodities/macro** (8: WTI, GOLD, SILVER, NATGAS, DXY, TNX, VIX, BTC) the other. A `group` field on `MARKET_SYMBOLS` partitions them; `MarketsTape` takes `group` + `reverse` (flips one via `animation-direction: reverse`) + `showMeta` props. **HO 185: dual tapes on EVERY page.** Both `HomeHeader` and `HeaderBar` now mount the shared **`<DualMarketsTape />`** (the equities → / commodities ← pair in a `.markets-tape-block`); HeaderBar's mount replaced HO 154.2's single combined `<MarketsTape />`. The no-group combined form (`<MarketsTape />` with no `group`) is now unused but left in place. All consume the same `MarketsTapeClient`, so live/stale/no-data states — and the HO 183 cycling AS OF stamp (bottom tape, one per page) — behave identically everywhere. Feeds off `getLatestMarketTicks()` (HO 142) — **17 MarketTicks as of HO 178** (HO 168 had 8, HO 177 → 14, HO 178 added silver/natgas + re-added DXY, dropped copper). Newly-added symbols show `—` (no arrow/change) until the next trading session gives them a prior-session reference.
