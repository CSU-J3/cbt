...
- **Stooq** (`https://stooq.com/q/l/`) — markets ticker (HO 142 → 168 → 177 → 178): the equity indices/ETFs SPX, NDQ (`^ndq`), DOW (`^dji`), and the ITA/XLK/XLV/XLF/XLE/XLI sector ETFs, plus WTI, GOLD (`xauusd` spot), silver (`xagusd`), natural gas (`ng.f`), DXY (`dx.f`), and BTC (`btcusd`). Public CSV endpoint, no auth, 15-20 min delayed. (DXY was dropped in HO 168, **re-added in HO 178**; copper was evaluated in HO 178 and dropped — no clean free USD/lb source on Stooq: `hgusd` is N/D, `hg.f` returns cents/lb.)
...
-- lib/markets.ts. HO 178: 17 symbols (HO 168 had 8, HO 177 grew to 14, HO 178
-- added silver/natgas + re-added DXY and dropped copper) — Stooq for the 9
-- equities (SPX/NDQ/DOW/ITA/XLK/XLV/XLF/XLE/XLI) + WTI/GOLD/SILVER/NATGAS/DXY/
-- BTC, FRED (end-of-day) for TNX + VIX (`VIXCLS`). ~50k rows/year (the 15 Stooq
-- symbols × ~13 intraday ticks × 252 days; TNX/VIX append once daily); history
-- retained as charting fuel.
...
- `/api/cron/markets` — **not on Vercel cron**, fires via GitHub Actions. **HO 172 split it into two workflows:** `markets-tick.yml` (intraday, `0,30 13-21 * * 1-5` — widened from 14-20 so it covers 9:30am-4pm ET in both DST regimes) hits `?source=stooq` for the 15 Stooq symbols (the 9 equities + WTI/GOLD/SILVER/NATGAS/DXY/BTC — HO 178); `markets-daily.yml` (after close, `0 22 * * 1-5`) hits the route with no param for all 17, incl. the FRED end-of-day symbols TNX + VIX (`VIXCLS`) which don't change intraday. The route filters `MARKET_SYMBOLS` by `?source`. Percent change is **vs the prior session close** — the prior-reference query filters `market_date < ?` (strict), so intraday ticks diff against the prior day's last row, not the prior 15-min tick. Appends one row per symbol, then `revalidateTag("markets")`. Per-symbol failures non-fatal (HO 139 chronicErr). Auth is the shared `CRON_SECRET` Bearer — the GitHub repo secret must equal Vercel's env var (a value mismatch 401s before `wrapCronRoute`, leaving no `cron_runs` row).
...
Bloomberg Terminal aesthetic. Dark monospace, dense rows, color-coded stages and topics. No light mode. Tailwind v4 only (no shadcn / no other libraries). Server components by default. Client islands are now numerous (31 `"use client"` components — grep `^"use client"` under `components/`; `CompetitiveRacesStrip` dropped out in HO 178 client→server, `CyclingTimestamp` added in HO 183, `SponsorHoverName` added in HO 192, `PrimaryRow` added in HO 203, the `CartogramShell` / `RaceMapCard` / `PrimaryMapCard` map islands added in HO 210; HO 194–200 + HO 202 added none) — filter/URL chips (`StageFilter`, `PartyFilter`, `StateFilter`, `CeremonialToggle`, `SortDropdown`, `SearchBox`, `SearchTabs`), the feed + dashboard accordions (`BillRowList` / `BillRow` / `BillExpandedPanel` / `TopStallsList`), interactive charts (`DashboardBubbleChart`, `PatternBubbleSVG`, `StageFunnel`), chrome (`MarketsTapeClient`, `CyclingTimestamp`, `ActivityTabs`, `Tooltip`, `MobileNavDrawer` — dormant), and row affordances (`WatchlistToggle`, `WatchStar`, `MediaAttentionCell`, `MemberHeader`, `RaceIncumbentCard`, `StateFlag`, `SponsorPhoto`, `SponsorHoverName`). See `### Server / client split` for the canonical set.

### Pages

- `/` — dashboard (HO 131 foundation, evolved through HO 133/134/132/149/**150**). HO 150 reflowed the layout per spec 1. **`HomeHeader`** chrome-stacks: (1) **masthead** (single-column full-width as of HO 162; **HO 185 replaced the brand prompt with the unified breadcrumb path**) — `Congress Terminal:\119TH\Dashboard>_`, rendered by the shared `BreadcrumbMasthead` (mono **36px on desktop** via `@media (min-width:1024px)` scoped to `.home-header-prompt-row .terminal-prompt`; holds 18px at 700–1023 and grows to 24px `<700`; segment text `--text-primary`, the path glyphs `:\` `\` `>` in `--accent-amber`, true tight shell spacing with no spaces around separators); **HO 178's LEAD prose still sits to the RIGHT** (`--text-secondary` ~14px, full-wrap, NO line-clamp/ellipsis/max-width — the row height follows the prose; removed entirely `<700` via `.home-header-lead { display:none }`). A single blinking `_` caret (`--accent-amber-bright`, 1.1s `step-end` opacity) rides the **path end (`…Dashboard>_`) at ALL widths** — HO 185 collapsed HO 178/179.1's per-breakpoint prose-caret / title-caret split into this one path caret (the prose no longer carries its own). The caret is now **one of three named motion exceptions** alongside the tape marquees and the HO 183 cycling timestamps; (2) `· LAST SYNC h:MM AM/PM <ZONE> · N BILLS TRACKED` meta line (11px `--text-muted`; 12-hour as of HO 169; **the time CYCLES zones ET→CT→MT→PT→UTC as of HO 183** via `CyclingTimestamp` — see "Date formatting" / the third motion exception); (3) **two counter-scrolling HO 178 markets tapes** (equities → / commodities ←, in a `.markets-tape-block`) between the masthead block and the nav; (4) **full-width nav** — seven `NAV_ITEMS` (Dashboard · Bills|News · Members · Races · Patterns · Reports · Watchlist; Feed was renamed **Bills|News**, and **Races** was added as its own item between Members and Patterns); active state amber-bright + 2px amber underline. Reports is a top-level entry as of HO 150; `pathToNavKey("/reports")` is special-cased to return `"reports"`. HO 154.5 extended `pathToNavKey` with prefix matchers so every dynamic-segment detail route highlights the right top-nav tab: `/bill/*` → Bills|News (key `feed`), `/members/*` / `/committee/*` → Members, **`/races` + `/race/*` → Races**, and **`/primaries` → Races** too — **HO 173** split Races + Primaries out of the Members group into their own **`races` GROUP_TABS sub-nav group**, so `/races` and `/primaries` now render the electoral sub-strip (**Races · Primaries**) and both light the Races top-nav tab; the Members group shrank to **Members · Committees**. The group key is `"races"` (not `"electoral"`) so it unifies with the existing top-nav `races` NavItem — `pathToNavKey`'s group loop returns `"races"` for both `/races` and `/primaries` with no extra matcher (the explicit prefix matcher only catches `/race/[id]` detail). `/reports/*` → Reports. Each detail page now passes its actual URL as `basePath` to HeaderBar (e.g. `<HeaderBar basePath={/bill/${bill.id}} />`). The first entry `⌂ DASHBOARD` (HO 140) is amber-highlighted on this page. **HO 162 moved the STAGES + TOPICS legend out of the masthead** (killing the wide-viewport void — no tall right-rail driving row height); **HO 167 then split it into per-chart keys** — `StageKey` and `TopicKey` render the swatch rows (`.panel-key`) at the **top of their own panels** (under the panel label, above the chart body), so each key sits with the chart it decodes. The interim HO 162 footer (`.home-footer` / `.color-key-footer` / the `ColorKeyStrip` component) was **retired** in HO 167. (The `?` `LegendBadge` that briefly held the PARTIES / BILL TYPES / ACCENT reference was **removed in HO 179.1** — the party-dot / bill-type meanings read as self-evident; its function was deleted from `ColorKeyStrip`, which now exports only `StageKey`/`TopicKey`.) Below the chrome stack, **HO 178 reflowed the body to 56/44** (reversing HO 150's equal 1fr/1fr grid). The full-width `<ReportSnapshot />` band (`.home-snapshot-slot`, HO 169 — see its note below) stays above the grid; below it a single `.home-grid` is **`56fr 44fr` (16px gutter)**. **LEFT column (56%)** stacks BREAKING → STAGE DISTRIBUTION → TOPIC DISTRIBUTION; **RIGHT column (44%)** stacks COMPETITIVE RACES (2×2) → ACTIVITY. **BREAKING is now compact rows** (`BreakingRow`: ID · truncated headline · age) with a hover-expand overlay that overruns rightward into the races column and dims it to 0.4 via CSS `:has()` (replaced the `showFullHeadline` full-width strip; the long-headline cross-column overrun is structurally in place but **not yet visually confirmed on a long-enough headline** — a deferred follow-up). **Competitive races are a 2×2 card grid with a ticker-style hover popover** (`CompetitiveRacesStrip`, now a **server** component — the HO 166/170 click-to-expand confined drawer + `useSingleOpenPanel` are retired): each card is a `<Link>` → `/race/[id]`, and hovering reveals an absolute, opaque (`--bg-row-hover`) popover rendering `RaceHubBody` in **`preview`** mode from **server-prefetched** hub data (no client `/api/race/[id]/hub` fetch; the endpoint is left in place, unused by this surface). Senate-led mix (top 2 Senate + top 2 House by competitiveness via `getMostCompetitiveRaces`); each card = seat label (carries HOUSE/SENATE) + incumbent (party dot + name, "OPEN SEAT" when null) + topline multi-source `RatingChip size="sm"`. STAGE DISTRIBUTION (funnel) + TOPIC DISTRIBUTION (bubbles) each carry their HO 167 color key (`StageKey` / `TopicKey`, `.panel-key`) at the top under the panel label; the RIGHT column's `ActivityTabs` (ACTIVITY default + TOP STALLS) — **HO 164 made both tabs' rows click-to-expand into the full `BillExpandedPanel`** (summary + meta instant, committees/news on fetch, action chips), single-open within each tab and reset-on-switch (the inactive tab unmounts); ACTIVITY renders `<BillRowList compact>`, TOP STALLS keeps its `[chip][title][Xd]` layout via the new `TopStallsList` wrapper. The HO 131/133/134 no-scroll `height: calc(100vh - var(--home-header-height) - 24px)` is gone — the dashboard scrolls if content exceeds viewport (a conscious reversal so the bubbles can read at full half-width per spec 1; answer-above-fold is preserved by masthead+tape+BREAKING). The `.home-snapshot-slot` (HO 150-reserved) holds a slim full-width `<ReportSnapshot />` (HO 153) — `WEEKLY REPORT <date> · <derived lead> · read full →` — now a full-width band **above** the 56/44 grid (HO 178; pre-178 it was a full-width band above the then-full-width BREAKING strip, HO 169). BREAKING (72h) is now the top panel of the LEFT column rather than a full-width strip under the report. The previous-weeks archive row (`PREVIOUS · <date> · … · all →`) was **removed from the dashboard in HO 159** as redundant — the `/reports` page already lists every past week — so `getDashboardReportSnapshot()` dropped its `previousDates` field (query is now `LIMIT 1`, returns `{ latest }` only) and the `.home-snapshot-prev*` CSS rules were deleted. **No separate mobile clamp was added** (HO 159 Phase 1 flagged the premise): the lead is already a 2-line block — `.home-snapshot-lead` is `-webkit-line-clamp: 2` and `extractReportLead` caps it at 180 chars — so acceptance's `<1024` "clamp + link to REPORTS" intent is met by the existing 2-line lead + `read full →`. It's a pointer, not a re-synthesis — the masthead prompt already carries the synthesized week-lead; the snapshot points at the written report and surfaces its lead via `extractReportLead`. Returns `null` (slot stays empty) when zero reports exist. Backed by `getDashboardReportSnapshot()` in `lib/queries.ts`. Bubbles still drive `?stage=` / `?topics=` click-to-filter (HO 132 in-place rebase): ACTIVITY and BREAKING both rebase to the filtered slice; bubble sizes rebase when STAGE is selected; the funnel rebases when TOPIC is selected. `ActiveFilterStrip` shows the active filter with `× CLEAR` and `VIEW IN /bills →` links. `await searchParams` opts the route into dynamic rendering. **DashboardBubbleChart** internals (HO 132/132.1): `"use client"` island using `d3-hierarchy::pack`, `value = Math.max(count, 1)` for sqrt-faithful area encoding, soft-nav via `router.push(href, { scroll: false })` on primary click; `<a href>` wrapper around each `<g>` preserves cmd/middle-click new-tab. Zero-count bubbles render at min radius 12px, `fill-opacity 0.35`, no click. Selected bubble gets `.dashboard-bubble.selected` with `--accent-amber-bright` stroke 2.5 + full opacity. HO 132.1's slide-out drawer was dropped — the dashboard rebases in place. `--home-header-height` is now dead code (the no-scroll calc that used it is gone).
- `/bills` — unified BILLS|NEWS feed (HO 151 restructured; **route renamed `/feed` → `/bills` in HO 184** to match its identity — `/feed` 308-redirects in `next.config.ts` with the query string preserved, so bookmarks/deployed URL survive; the GROUP_TABS group key stays `"feed"` internally so `pathToNavKey` is unaffected, with an explicit `pathToNavKey("/bills") → "feed"` matcher added since `/bills` no longer has a sub-nav tab — see the double-nav note below). **HO 185 removed the per-page `page-masthead` H1** (`Bills:\>`/`News:\>`); the unified breadcrumb path is now the only masthead and tracks the toggle — `Congress Terminal:\119TH\Bills>_` in bills mode, `…\News>_` in news mode (the `mode`/`presidentAlias` props on `HeaderBar` drive the segment, server-side). A single `SegmentedToggle` (`?mode=bills`|`?mode=news`, default bills) swaps the filter bar, row component, and count chrome; page resets on every filter or mode change. Per-mode params don't collide; the other mode's params persist in the URL across switches (no aggressive clear). `BILLS` (`?mode=bills`): **25/page** (`FEED_PAGE_SIZE`, HO 187 — down from 100, not the 50 this entry long claimed), `BillRowList` accordions (HO 148). **HO 187/189 restructured inner pages into 4 chrome bands** (see the HeaderBar / inner-chrome note): (1) the title bar (path + inline filter-aware sync + nav), (2) the **control strip** — BILLS|NEWS toggle + `StageFilter` (ALL STAGES, includes `president`) + chamber `SegmentedToggle` (ALL/HOUSE/SENATE) + a **compact ⌕ `SearchBox`** (`flex:1`, max 230px) + **INCLUDE CEREMONIAL** + `SortDropdown` pinned right (search + ceremonial moved here out of the old HeaderBar masthead), (3) the **topics band** — the feed sub-nav Changes·President·Reports + a divider rule + the multi-select `TopicFilter` chips (comma-list `?topics=…`), (4) the **legend + pagination** band (`StageLegend` + `Pagination`). The markets tape was removed from inner pages in HO 187, then **restored app-wide in HO 202** (the dual `DualMarketsTape` is mounted once in `HeaderBar` below the title bar, so it's back on `/bills` and every inner page). BILLS-only params: `stage`, `sponsor`, `chamber`, `ceremonial`, `cluster`, `q`, `topics`, `sort`. The only structurally shared param is `page`; `topics`/`sort` are BILLS-only but preserved in the URL across a mode switch (round-trip symmetry), not consumed by NEWS rendering. (Corrected in the HO 160 sweep — the prior "shared `topics`/`sort`/`page`" framing conflated URL-preservation with cross-mode use.) `NEWS` (`?mode=news`): 50/page, `NewsRow` list inside the same `news-header-row` chrome the standalone `/news` used, `NewsFilters` chips for `SOURCE` (politico/the_hill/roll_call) + `WINDOW` (24/72/168/720h, default 72h) + single-select `TOPIC` (`?topic=…` singular, distinct from BILLS' plural) + a clearable `?bill=<id>` pill that scopes to one bill (drops window + confidence gate while scoped — same as HO 130's per-bill semantics). NEWS-only params: `source`, `topic`, `window`, `bill`. SORT is a static "most recent" label — no dropdown shell. `HeaderBar` runs in `pageTitle="News mentions"` + `pageCount=mentions` mode for NEWS, dropping the search box. Backed by `getNewsFeed({source?, topic?, windowHours?, billId?, page?, pageSize?})` which reuses `getBreakingNewsForHome`'s dedup-by-article-key + confidence floor (0.7) when not bill-scoped, paginates via OFFSET, tag `news-breaking`. Companion sanitizers: `sanitizeNewsSource`, `sanitizeWindowHours`. **President alias absorbed:** `/president` redirects to `/bills?stage=president`; when `stage=president` is the sole active stage AND no explicit `?sort` override, the feed page re-applies the legacy `daysSinceMode="desk-time"` desk-time column AND switches the SQL `ORDER BY` to ASC (oldest-at-desk first) — the column and the order ship together so the visible signal stays coherent. `FeedFilters.direction?: "asc"` carries this through `getFeedBills`; it's internal-only, never written by URL params and never surfaced in `SortDropdown`. `getPresidentBills` / `getPresidentCount` were deleted in HO 154.1 as orphans of this redirect. **News alias absorbed:** `/news` redirects to `/bills?mode=news` (and `/news?bill=<id>` to `/bills?mode=news&bill=<id>`); `MediaAttentionCell` points directly at the new target. (The `/news` + `/president` alias `redirect()`s were retargeted straight to `/bills` in HO 184 to avoid a `/news → /feed → /bills` double-hop.) **Double-nav resolved (HO 184):** the BILLS|NEWS `SegmentedToggle` is the canonical mode switch; the `feed` GROUP_TABS row previously *also* carried Bills + News tabs (a redundant second way to switch the same mode), so they were dropped — the feed sub-nav now reads **Changes · President · Reports** (sibling/derived views; Reports kept as a deliberate pre-existing special-case). Accepted tradeoff: in bills/news mode no sub-nav tab is active.
...
- `/president` — server redirect to `/bills?stage=president` (HO 151). The standalone page is gone; the feed re-applies `daysSinceMode="desk-time"` + oldest-at-desk-first SQL ordering when `stage=president` is the sole active stage with no explicit `?sort`. Existing bookmarks resolve to the same desk-time view they did before.
...
- `/members` — distinct sponsors aggregated from `bills`, evolved through HOs 67/89 and reorganized for spec 7 in HO 152. Filter bar (single row): page title + count, then ALL/HOUSE/SENATE `SegmentedToggle` + `PartyFilter` (R/D/I) + `StateFilter` + VOLUME/PASS RATE `SegmentedToggle` (drives list ranking via `?sort=volume|passrate` per `getMembersRanked`'s ORDER BY swap) + **INCLUDE CEREMONIAL** (`CeremonialToggle`, **relocated here from the masthead chrome in HO 195** when `/members` migrated to `pageOwnsControls` — it has a real effect, `billsAggCte(includeCeremonial)` shifts the totals/ranking, so it was relocated not removed). **List rows (HO 196):** `# · MEMBER (name + party-colored bracket) · [BAR] · RATE · ENACT · BILLS` with a thin 9px `--text-dim` border-bottom header row. **ONE bar per active sort** (never two): VOLUME (the default landing — no `?sort` param) = party-colored bar scaled to the page `maxVolume`; PASS RATE = green `--stage-enacted` full-width track (= enacted %, 0% leaves it empty). The middle (bar) header label swaps **BILLS ↔ PASS RATE** with the toggle; figures are RATE (green >0 / dim at 0), ENACT (`N ✓`), BILLS (dim, emphasized `--text-primary` under volume sort). Default stays VOLUME — the sort logic is unchanged (the design's "pass rate default" described the bar STATE, not the landing view). The metric toggle replaced the HO 67 `SponsorSortToggle` (which is gone); the chamber control on this page is a local `SegmentedToggle` instance so the same component drives both the row list and the two scatters. The old bespoke `ChamberToggle` was deleted in the HO 154.3 cleanup; every chamber and metric toggle in the app (feed, changes, stale, watchlist, members) now mounts the shared `SegmentedToggle` directly with `CHAMBER_SEGMENTS`. **Two side-by-side scatters** (`MemberProductivityScatter chamber="house"` / `chamber="senate"`, HO 152) sit above the list and read directly off the chamber URL state (only the active half renders when a chamber is selected). Each chart is hand-rolled SVG (HO 67's lineage) with three readability fixes: **log x-axis** (`xLog(v) = log10(v + 1)`, ticks 1/10/100/500, integer labels), **y zoomed to 0–30%** (`Y_ZOOM_MAX = 0.30`), and an **out-of-range `▲` marker** above any clamped dot, in the party color. Tooltips stay native `<title>` (HO 147 isn't directly compatible inside SVG without `foreignObject`; the rich-content benefits don't load-bear on hover dots) carrying `name · N bills · M% pass rate · K enacted`. Dot opacity `0.7`. **Labels (HO 197, replacing the old top-5-by-volume + top-5-by-pass-rate rule that caused the Norton/Hagerty pileups):** a dot earns a short-name label only above a **threshold** (passRate > 10% OR billCount > 100), then a **greedy collision dodge** (salience-sorted, suppress any label within ~16px of an already-kept one) collapses residual stacks; suppressed dots keep their hover. The **0%-pass mass gets a seeded ±~3px vertical jitter** so the y=0 line reads as a density band, not a smear — derived deterministically from the sponsor key (NOT `Math.random`: this is a per-request server component that would re-jitter every load), presentational only (offsets the rendered `cy`; the `<title>` + `<Link>` stay bound to TRUE row data). Labeling is per-POINT with a one-label-per-member guard. Dots link to the member hub via the existing bioguideId wrap. **Pass rate on the scatter = enacted/total** (HO 152 calibration so dots actually fall inside the 0–30% zoom band), distinct from HO 67's advanced/total which clusters dominantly above 30% — the list view's "pass rate" column already used enacted/total, so the chart now matches. Backed by `getSponsorProductivity()` — **HO 199 regrouped it to `GROUP BY b.sponsor_bioguide_id` alone, sourcing display name/party/state/chamber from the canonical `members` JOIN** (it was `GROUP BY sponsor_bioguide_id, sponsor_name, …`, which split one member into multiple dots when their bills carried name variants under one bioguide — e.g. Begich 27+7; scatter dots ~557→536 as ~21 split members merged; 0 null-bioguide bills so no fallback needed). The HO 197 per-point/one-label guard is now belt-and-suspenders (the data is deduped upstream). Sponsors whose `bioguide_id` doesn't resolve to a current member silently drop from both scatters. Click any row to inline-expand a `SponsorExpandedPanel` — **HO 198/200 rebuilt it from the ~650px vertical stack into a ~280px 3-column card matching the HO 191 bill panel** (grid `210px · minmax(0,1.5fr) · minmax(0,1fr)`, `.sponsor-card-grid`/`-rail`/`-mid`/`-right`):
  ...
  - **RIGHT:** COMMITTEES · N (HO 152, `getMemberCommittees`), capped ~8 + CHAIR/RANKING badges + `↳` subcommittees + "+N more subcommittees".
...
- `/patterns` — bill pattern visualization (HO 128, supersedes the HO 114 table render). Two-column layout: left column holds a `PatternBubbleSVG` (hand-rolled SVG, one circle per regex pattern from `lib/cluster-patterns.ts`) sized via `d3-hierarchy::pack` with sqrt-area scaling, colored by `% past committee` on a `--text-dim` → `--stage-enacted` gradient (stalled→moving; the noise-vs-signal lens). `PatternLegend` below renders the gradient strip. Selection is URL-driven via `?selected=<slug>` (validated by `sanitizeClusterId`); clicking a bubble pushes the param with `scroll: false`, re-clicking clears it. When selected, `PatternDrilldownPanel` slides in below the legend with a headline line (`PATTERN: name · N bills · X% past committee · K enacted · Y% ceremonial`), top-5 sponsor mini-bar-chart (party-colored, name-truncated), and a `[View all N bills in feed →]` link to `/bills?cluster=<slug>` (HO 51 drill-out convention). Right column is contextual: when nothing is selected, the legacy `cluster-row` table renders as a fallback (links push `?selected=` rather than navigating away); when selected, it shows the top-10 most-recent bills via reused `BillRow compact` with pre-resolved `getWatchedBillIds` membership. Backed by `getClusterStats()` (extended with `pastCommittee/enacted/ceremonial`) and `getClusterDrilldown(clusterId)` (top sponsors + recent bills + headline), both `unstable_cache` tagged `bills`. **HO 199:** the top-5-sponsors sub-query now `GROUP BY sponsor_bioguide_id` (was `GROUP BY sponsor_name, sponsor_party`, which split a member with name variants under one bioguide — the same dedupe applied to `getSponsorProductivity`), keeping `MAX(sponsor_name)` because this panel's `shortSponsor()` expects the "Last, First" shape. Mobile (`< 700px`): columns stack, SVG shrinks to 360px. Internal column (`bills.cluster_id`) and helper names keep `cluster*` naming per HO 114.
...
- `/committees` — committee index (HO 144). Flat list of every `is_current = 1` committee + subcommittee, joined to per-committee `members_count` + `recent_count` (distinct bills with any `committee_bills.activity_date` in the trailing 30 days) via two LEFT JOIN subqueries in `getCommitteesIndex`. URL state: `?chamber=house|senate|joint` (single value, validated by `sanitizeCommitteeChamber`; missing = all) and `?sort=activity|name|members` (default `activity`, validated by `sanitizeCommitteeSort`). Sort is computed in SQL — `ORDER BY recent_count DESC, c.name ASC` for activity (the framing question is "which committee is most active right now," not all-time volume; House Energy and Commerce / Judiciary lead in steady state); `c.name ASC` for name; `member_count DESC, c.name ASC` for members. Subcommittees included in counts by default — no "top-level only" toggle in v1. Six-column grid (name · chamber · type · 30d · members · `↳ sub` tag) inlined as a single Tailwind utility class on the page; no globals.css addition needed because this is the only consumer. Chamber and Sort segmented filters mirror `SegmentedToggle`'s visual idiom but are inlined server-rendered links because the shared `CHAMBER_SEGMENTS` is ALL/HOUSE/SENATE only (no `joint`) and `SortDropdown` is typed to the feed `SortKey`. `HeaderBar` runs in `pageTitle="COMMITTEES" + pageCount` mode. Cached under tag `committees`; `/api/cron/committees` already calls `revalidateTag("committees")` per HO 143.
...
- **`HomeHeader`** — dashboard (`/`) only. Holds the unified breadcrumb path (`BreadcrumbMasthead` → `Congress Terminal:\119TH\Dashboard>_`, HO 185 — was the `Congress Terminal:\>` `TerminalPrompt`), the lead-in prose to its right, the blinking cursor at the path end (`.home-cursor-caret`, now one of three motion exceptions), the `· LAST SYNC … · N BILLS TRACKED` subhead (the time cycles zones, HO 183), the shared `DualMarketsTape`, and its own `.home-header-nav`.
- **`HeaderBar`** — every other page. **HO 185** replaced the `CBT // 119th Congress` title with the breadcrumb path (`Congress Terminal:\119TH\<Section>[\<Detail>]>_`, via `BreadcrumbMasthead` + `breadcrumbSegments(basePath, …)`); the dead `variant="dashboard"` branch was deleted. **HO 187 consolidated inner-page chrome into bands** and removed the markets tape from `HeaderBar` (dashboard-only again — reversing HO 185's dual-tapes-everywhere); the nav moved back beside the path (the HO 185 `.header-nav-row` is gone), `SearchBox` + `CeremonialToggle` moved out to the page's control strip (band 2), and the path was bumped to **20px** (> the **13px** nav — title dominates). **HO 202 SUPERSEDED the tape removal** — `<DualMarketsTape />` is mounted once in `HeaderBar` below the title bar, so the dual tape is back on every inner page (matching the dashboard's masthead → tape → nav order; the dashboard's `HomeHeader` tape is a separate mount, no double-up). **HO 189 pulled the standalone sync strip inline into the title bar**, after the cursor: `…>_ · N BILLS · UPDATED MT` (filter-aware — `N OF M BILLS · "q" · sponsored by … · UPDATED MT` when filtering; `updated` is static `formatLastUpdated` MT, **not** the cycling stamp — that's dashboard/tape-only). So `HeaderBar` now renders just the **title bar** (path + inline sync + nav); bands 2–4 (control / topics / legend+pagination) are page-rendered. **Inline-sync degrade:** the sync is the lowest-priority element — `flex:1 1 0` (basis 0 so its content never forces the nav to wrap) + `min-width:0` + ellipsis, so it truncates/collapses to empty BEFORE the nav (protected, `flex-shrink:0`, pushed right by the sync's grow) wraps or the path gives; the deepest paths wrap the nav under as the last resort. The sync is empty on detail pages (deep paths — corpus count isn't page-specific; no dangling ellipsis). `/bills` · `/changes` · `/stale` pass `pageOwnsControls` (HO 187) so HeaderBar suppresses its transitional legacy search+ceremonial band and each renders its own control strip. **HO 195 migrated `/members` onto `pageOwnsControls` too — so every page is now on the inner-page chrome pattern and the transitional legacy band no longer renders for any page** (the band + prop stay in `HeaderBar` only as the now-unexercised fallback). `/members` keeps `feedFilters` (for the inline sync line) but drops the redundant legacy `search bills…` input and relocates INCLUDE CEREMONIAL into its own filter row. **Members-only masthead divergence (HO 195):** two props that ONLY `/members` sets — `cursorAtEnd` moves the blinking `_` off the breadcrumb `>` (suppressed there via `BreadcrumbMasthead`'s new `cursor` prop, default `true` = glued everywhere else) to trail the FULL inline sync string, and `liftSyncContrast` lifts the sync run to `--text-muted` + the count to `--text-secondary`. Bills/every other page are unchanged (cursor stays glued to `>`, dim sync) — the divergence is deliberate and commented so it isn't "fixed" as drift.
...
Nav is **one flat row of 7 group landings** (`NAV_ITEMS` in `HeaderBar.tsx`: Dashboard · Bills|News · Members · Races · Patterns · Reports · Watchlist — Feed→Bills|News rename, Races added between Members and Patterns). The old secondary STALE·CHANGES·PRESIDENT·WATCHLIST row was removed by HO 134; those destinations live inside `GroupTabs` on the landing pages.
...
|---|---|---|---|---|
| HomeHeader prompt (`.home-header-prompt-row .terminal-prompt`) | **36px** (HO 162) | 18px | 24px | **Grows** — the dashboard hero; HO 162 bumped the ≥1024 brand 18→36px (the 700–1023 base and <700 24px grow are unchanged) |
...
| HeaderBar breadcrumb path (`…>_ · sync`, HO 185/187/189) | **20px** | 20px | 20px | HO 187 made the path the title (> the 13px nav, 10px inline sync); the sync truncates first, the nav wraps under for the deepest paths |
| HeaderBar count line | 11px | 11px | 11px | Holds; **wraps** to 2 lines on narrow, never truncates (no nowrap/truncate on it) |
| HeaderNav / home-nav | 16 / 14–16px | same | same | Both **wrap** when cramped — home-nav (`.home-header-nav`, CSS) and HeaderBar nav (`.header-nav` = `flex flex-wrap`, HO 161). Static, no scroll-x. Drawer dormant — see above. |
...
Markets ticker tape(s). **HO 178: the dashboard mounts TWO counter-scrolling tapes** (`HomeHeader`, in a `.markets-tape-block` between the masthead and the nav) — **equities** (9: SPX, NDQ, DOW, ITA, XLK, XLV, XLF, XLE, XLI) scrolling one way and **commodities/macro** (8: WTI, GOLD, SILVER, NATGAS, DXY, TNX, VIX, BTC) the other. A `group` field on `MARKET_SYMBOLS` partitions them; `MarketsTape` takes `group` + `reverse` (flips one via `animation-direction: reverse`) + `showMeta` props. **HO 185 mounted dual tapes on EVERY page; HO 187 removed them from inner pages (dashboard-only); HO 202 RESTORED them app-wide** — the dual tape is back on every page. The shared **`<DualMarketsTape />`** (the equities → / commodities ← pair in a `.markets-tape-block`) is mounted in **two** places: `HomeHeader` (dashboard, between the masthead and nav) and `HeaderBar` (HO 202, once, below the title bar → reaches every inner page). No double-mount: the dashboard uses `HomeHeader`, never `HeaderBar`. The no-group combined `<MarketsTape />` form is unused but left in place. So the tape's `MarketsTapeClient` + the HO 183 cycling AS OF stamp (bottom tape) now ride **every page** again (the HO 187 "dashboard-only" state is superseded). The inner-page **title-bar sync** still shows a static `UPDATED … MT` (`formatLastUpdated`) — that's separate from the tape's cycling AS OF and unchanged. Feeds off `getLatestMarketTicks()` (HO 142) — **17 MarketTicks as of HO 178** (HO 168 had 8, HO 177 → 14, HO 178 added silver/natgas + re-added DXY, dropped copper). Newly-added symbols show `—` (no arrow/change) until the next trading session gives them a prior-session reference.
...
- **Motion exception:** the marquee is the one deliberate exception to the dashboard's cursor-blink-only motion rule. Double-track `translateX 0 → -50%` linear infinite for a seamless wrap. **HO 168** made the speed count-agnostic: JS measures the track-half width and sets `animationDuration = halfWidth / 40px-per-sec`; the `-50%` lands on exactly one half-width only because **HO 172 removed the 24px `padding-right`** that used to push it to `-(W+12)` and jump the seam every loop. **HO 172** replaced the click pause toggle with **hover-to-pause** (CSS `.markets-tape-track:hover { animation-play-state: paused }`) — the `cbt-tape-paused` localStorage and the ⏸/▶ button are gone. `@media (prefers-reduced-motion: reduce) { animation-play-state: paused }` still wins. **HO 178** added the commodities tape's opposite direction (`.markets-tape-track--reverse { animation-direction: reverse }`). **Stacking (HO 179.1):** the marquee's animated `transform` makes `.markets-tape-track` a stacking context that traps the hover popover; the fix is `.markets-tape { z-index: 30 }` (above the nav on every page) plus the dashboard `.markets-tape-block` ordering the top tape z-31 over the bottom z-30, so either tape's downward popover paints above the other tape + the nav (no portal).
- **Three render states:**
  - *Live* — double-track scrolls, each tick renders `SYMBOL VALUE ▲/▼ ±N.NN%` (arrow + change% in `--market-up` / `--market-down`; flat = `•` in `--text-secondary`; a null-change symbol renders just `SYMBOL VALUE`). Hovering a tick pauses the track (HO 172) AND shows a CSS-absolute **hover-expand popover** (HO 175–177: `fullName` + group label + change% + as-of date, on `--bg-row-hover`). `AS OF h:MM AM/PM <ZONE>` right-pinned (12-hour; **the zone cycles ET→CT→MT→PT→UTC as of HO 183** via `useZoneCycle`/`formatInZone`) on a solid `--bg-panel` meta with a left fade strip (HO 179) — **only the bottom tape renders it** (one shared stamp, `showMeta={false}` on top — HO 179.1; **on every page again as of HO 202's app-wide tape restore**).
  - *Stale* (latest `tickedAt` older than 26h) — animation stopped, single static track, values in `--text-dim`, arrows dropped, `AS OF h:MM AM/PM <ZONE> · STALE` with the flag in `--accent-amber` (the zone still cycles per HO 183; reduced-motion pins MT). Weekend STALE is expected and correct (no trading-calendar logic in v1; honest AS OF stamp carries the age).
...
- The dashboard at `/` is server-rendered apart from a small set of client islands: `DashboardBubbleChart` (URL-driven bubble navigation), `ActivityTabs` (local tab state), `MarketsTapeClient` (HO 149 marquee + pause), and the accordion wrappers `BillRowList` (HO 148 — feed-shaped pages, and as of **HO 164** the dashboard ACTIVITY ticker via `<BillRowList compact>`) plus `TopStallsList` (**HO 164**, the dashboard TOP STALLS accordion). Charts (`TopicMixByChamber`, `BillsTimeSeries`, etc.) remain static server components — but note `StageFunnel`, despite being a chart, is a `"use client"` island.
- The full `"use client"` set is 31 components (grep `^"use client"` under `components/` to refresh — minus the two hook files `useSingleOpenPanel.ts` / `use-watch-toggle.ts` that the same grep catches; `PrimaryRow.tsx` also co-locates a second client export, `PrimaryExpandProvider` (the primaries single-open context), in the same file — one file, not a separate island; this list drifted to just two for several handoffs before the HO 160 sweep, then carried an inaccurate 28-name list until the HO 161 sweep; `TopStallsList` added in HO 164, `CompetitiveRacesStrip` added in HO 166 then **dropped in HO 178** when the races strip went client→server; `CyclingTimestamp` added in HO 183; **HO 187–189 added none** — the topic-hover is CSS, the bill-row enrichment is server/column-level, the inner-chrome rework is layout; **HO 191 added none** — the stage pipeline + avatar are nested inside `BillExpandedPanel`/`SponsorHoverName`, not new islands; **HO 192 added `SponsorHoverName`**; **HO 194–200 added none** — 194 refined `SponsorHoverName`, and 195–200 (Members chrome/list/scatter/dedupe/3-column card) are all server-component, page, or query changes; **HO 202 added none** — the tape mount went into existing `HeaderBar`; **HO 203 added `PrimaryRow`** for the primaries candidate-field row + single-open expand; **HO 210 added the `CartogramShell` / `RaceMapCard` / `PrimaryMapCard` map islands** for the map-first /races + /primaries surfaces): filter/URL chips `StageFilter` `PartyFilter` `StateFilter` `CeremonialToggle` `SortDropdown` `SearchBox` `SearchTabs`; feed + dashboard accordions `BillRowList` `BillRow` `BillExpandedPanel` `TopStallsList`; interactive charts `DashboardBubbleChart` `PatternBubbleSVG` `StageFunnel`; chrome `MarketsTapeClient` `CyclingTimestamp` `ActivityTabs` `Tooltip` `MobileNavDrawer` (dormant); and row affordances `WatchlistToggle` `WatchStar` `MediaAttentionCell` `MemberHeader` `RaceIncumbentCard` `StateFlag` `SponsorPhoto` `SponsorHoverName` `PrimaryRow`; and the race/primary map islands `CartogramShell` `RaceMapCard` `PrimaryMapCard`. `TopicFilter`, `SegmentedToggle`, `NewsFilters`, `Pagination`, `MemberProductivityScatter`, `PatternDrilldownPanel`, `MarketsTape`, `BreadcrumbMasthead`, `DualMarketsTape`, `GroupTabs`, and `ActiveFilterStrip` are server components despite their interactive feel — they render `<Link>`s and let the router carry state. Most client islands exist only to push/replace URL state; `WatchlistToggle` is the lone POST.
...
- **`?` badge** (`variant="badge"`). 16px bordered mono box for panel/section-level help. First applied in `ColorKeyStrip`; now the standalone **`LegendBadge`** in the dashboard header (HO 162 extracted it; HO 167 retired the rest of `ColorKeyStrip`), popping the PARTIES / BILL TYPES / ACCENT body in one panel.
...
Initial HO 147 wiring: `ColorKeyStrip`'s `?` badge popped the legend body (now the header `LegendBadge`, HO 167); `StageLegend`'s `▸ COMMITTEE` chip was the original dotted-underline example (now joined by every other StageIndicator across the app via HO 154.6).
...
- `formatLastUpdated(iso)` → `h:MM AM/PM MT` (America/Denver, 12-hour as of HO 169). Still backs the inner-page `HeaderBar` "updated … MT" subhead, **which stays static MT — not yet cycling (deferred)**.
- **Cycling timestamps (`lib/zone-cycle.ts`, HO 183).** `formatInZone(iso, zone)` + the `useZoneCycle()` hook re-render a FIXED past instant through US zones **ET→CT→MT→PT→UTC** (4s/zone, 20s loop) — not a live clock. DST-correct via IANA `Intl.DateTimeFormat`; generic `ET/CT/MT/PT/UTC` labels (no DST-flapping EDT/EST). The active zone is **clock-derived** (`floor(Date.now()/ZONE_MS) % ZONES.length`), so the two cycling stamps — the dashboard masthead LAST SYNC (`components/CyclingTimestamp.tsx`, a client island) and the markets-tape AS OF (the bottom tape — **on every page again as of HO 202's app-wide tape restore**) — stay in sync with no shared ticker. (The inner-page title-bar sync still uses static `formatLastUpdated` MT, not the cycling stamp — that's separate from the tape's AS OF.) Reduced-motion → static MT. This is the **third named motion exception** (cursor blink + tape marquees + this).
