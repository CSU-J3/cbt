...
- `/primaries` — 2026 primary + runoff calendar (HOs 91-96/107/120). Server page; `GroupTabs` + `TerminalPrompt`. Backed by `getUpcomingPrimaries` / `getPastPrimaries`; URL params `?chamber` / `?party` (sanitized via `sanitizePrimaryChamber` / `sanitizePrimaryParty`) + `?page` (`PAGE_SIZE = 30`). Rows render via `PrimaryCalendarRow`. Reads `primaries` + `primary_candidates` (see "Runoff tracking" + the primaries cron notes).
- `/races` — competitive-races surface (HOs 62/71/72). Server page; `getRacesIndex` (+ `getUpcomingPrimaries` / `getPastPrimaries` for each race's primary date), rendered as `race-row`s. No URL params. Links into the `/race/[id]` hubs; the `ABS(rating_score) <= 1` competitive cut and the per-source `RatingChip` rendering match the "Race ratings" section.
