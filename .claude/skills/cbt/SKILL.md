...
- **Three render states:**
  - *Live* — double-track scrolls, each tick renders `SYMBOL VALUE ▲/▼ ±N.NN%` (arrow + change% in `--market-up` / `--market-down`; flat = `•` in `--text-secondary`; a null-change symbol renders just `SYMBOL VALUE`). Hovering a tick pauses the track (HO 172) AND shows a CSS-absolute **hover-expand popover** (HO 175–177: `fullName` + group label + change% + as-of date, on `--bg-row-hover`). `AS OF h:MM AM/PM UTC` right-pinned (12-hour as of HO 169) on a solid `--bg-panel` meta with a left fade strip (HO 179) — and on the dashboard **only the bottom tape renders it** (one shared stamp, `showMeta={false}` on top — HO 179.1).
  - *Stale* (latest `tickedAt` older than 26h) — animation stopped, single static track, values in `--text-dim`, arrows dropped, `AS OF h:MM AM/PM UTC · STALE` with the flag in `--accent-amber`. Weekend STALE is expected and correct (no trading-calendar logic in v1; honest AS OF stamp carries the age).
...
- The full `"use client"` set is 25 components (grep `^"use client"` under `components/` to refresh — this list drifted to just two for several handoffs before the HO 160 sweep, then carried an inaccurate 28-name list until the HO 161 sweep; `TopStallsList` added in HO 164, `CompetitiveRacesStrip` added in HO 166 then **dropped in HO 178** when the races strip went client→server): filter/URL chips `StageFilter` `PartyFilter` `StateFilter` `CeremonialToggle` `SortDropdown` `SearchBox` `SearchTabs`; feed + dashboard accordions `BillRowList` `BillRow` `BillExpandedPanel` `TopStallsList`; interactive charts `DashboardBubbleChart` `PatternBubbleSVG` `StageFunnel`; chrome `MarketsTapeClient` `ActivityTabs` `Tooltip` `MobileNavDrawer` (dormant); and row affordances `WatchlistToggle` `WatchStar` `MediaAttentionCell` `MemberHeader` `RaceIncumbentCard` `StateFlag` `SponsorPhoto`. `TopicFilter`, `SegmentedToggle`, `NewsFilters`, `Pagination`, `MemberProductivityScatter`, `PatternDrilldownPanel`, `MarketsTape`, `GroupTabs`, and `ActiveFilterStrip` are server components despite their interactive feel — they render `<Link>`s and let the router carry state. Most client islands exist only to push/replace URL state; `WatchlistToggle` is the lone POST.
...
- `formatLastUpdated(iso)` → `h:MM AM/PM MT` (America/Denver, 12-hour as of HO 169) for the header bar.
