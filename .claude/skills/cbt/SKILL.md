...
- `/` — dashboard (HO 131 foundation, restructured HO 133 layout pivot v2). Top-of-page `HomeHeader` is a 3-column band at ≥1280px viewport: **(Col 1) title block** capped at 700px containing the `Congress Terminal:\>` prompt (26px mono, `--text-primary` + `:\>` in `--accent-amber`) → LEAD prose with viewport-tiered line-clamp (3 lines ≥1920px, 2 lines 1199–1919, 1 line <1199 — font-size stays constant, visible line count is what scales) → `· LAST SYNC HH:MM MT · N BILLS TRACKED` meta (11px `--text-muted`) → `--border-soft` separator → 12-link nav row (every link carries HO 131 tooltip copy via `title=` + `aria-label`, source-of-truth `NAV_ITEMS` in `HeaderBar.tsx`). **(Col 2) Stage Distribution** — the same `StageFunnel` (6 bars + off-path footnote) pulled out of the grid into the header band. **(Col 3) Color Key** — `ColorKey` with 5 sections: Stages (6 chips), Topics (8 color groups), Parties (3 chips), **Bill Types** (8 chips colored by chamber-of-origin — house cyan, senate purple — each carrying the type acronym + full-name tooltip from `BILL_TYPE_LABELS`), and Accent (single-line note). Every chip has a `title=` tooltip sourced from `STAGE_LABELS` / hand-curated topic strings / party full names / `BILL_TYPE_LABELS`. Below 1280px the header reflows to a 2-row stack (title row 1, stage+color row 2); below 800px collapses to single column. Below the header sits a 2×2 `.home-grid` with `height: calc(100vh - var(--home-header-height) - 24px)` and `overflow: hidden` quadrants: **Row 1** — `BreakingStallsTabs` (client island with local `useState` toggle between BREAKING and TOP STALLS, reuses `.search-tabs` chrome) + ACTIVITY (`ActivityTicker`). **Row 2** — TOPIC DISTRIBUTION spans both columns (`grid-column: 1 / -1`), with inner `overflow-y: auto` until HO 132 swaps in the bubble. BREAKING + ACTIVITY both cap at **5 rows** with a `[ + N MORE → ]` expander footer (`.home-expander` class) linking to `/news` and `/changes` respectively. The expander reads N from `getBreakingNewsForHomeCount` (new HO 133 helper) and `getStageChangesCount` (existing). Primary target 2560×1440; scales fluidly to 1920×1080 without scroll. At 1440×900 the grid renders 2×2 and may scroll modestly per spec. Below 1200px relaxes auto-height; below 700px collapses to single-column stack. Accepts `?stage=<stage>` and `?topics=<topic>` query params for click-to-filter — single value each, validated against the Stage and Topic enums, invalid values silently dropped (no `q`/`sort`/`sponsor`/`cluster` on `/`). Selected bar/row renders at full opacity, others at 0.4. `ActiveFilterStrip` renders between the header and the grid when any filter is active, with `× CLEAR` (→ `/`) and `VIEW IN /FEED →` (→ `/feed?stage=…&topics=…`) links. Because `await searchParams` opts the route into dynamic rendering, `/` is no longer statically prerendered — the `bills`-tagged query cache still applies at the query layer. The `DashboardLead` standalone component was deleted in HO 131; `getDashboardLead()` is now called directly from `HomeHeader`.
