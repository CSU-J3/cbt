...
- `/api/sync` — 09:00 UTC — bills: sync + dashboard lead + news + trades + Monday weekly report. **Summarize was split out in HO 115** because it ate the shared 60s budget and starved every step behind it.
...
- **Vercel Hobby 60s cap + multi-step daily syncs = silent step-level failures unless each step is its own cron or has its own per-step timeout budget.** HO 115 lesson: `/api/sync` ran bill-sync → summarize → lead → news → trades → report all in one 60s function. The summarize step crept past its share of the budget around 2026-05-18 and quietly killed every step behind it — news ingestion went dark for four days with no error, just an absence (the only fingerprint was the `cron_runs` row stuck at `running`). The fix was to split summarize into `/api/cron/summarize` with its own budget; the same shape applies any time a daily cron grows past 2–3 sequential heavy steps. The sibling rule (per-step timeout) is the runner's 45s deadline + 15s per-bill `AbortController` so the new route's budget is structurally enforced, not just expected.
