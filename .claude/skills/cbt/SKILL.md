...
Bloomberg Terminal aesthetic. Dark monospace, dense rows, color-coded stages and topics. No light mode. Tailwind v4 only (no shadcn / no other libraries). Server components by default. Client islands are now numerous (27 `"use client"` components — grep `^"use client"` under `components/`; `CompetitiveRacesStrip` dropped out in HO 178 client→server, `CyclingTimestamp` added in HO 183, `SponsorHoverName` added in HO 192; HO 194–200 added none) — filter/URL chips (`StageFilter`, `PartyFilter`, `StateFilter`, `CeremonialToggle`, `SortDropdown`, `SearchBox`, `SearchTabs`), the feed + dashboard accordions (`BillRowList` / `BillRow` / `BillExpandedPanel` / `TopStallsList`), interactive charts (`DashboardBubbleChart`, `PatternBubbleSVG`, `StageFunnel`), chrome (`MarketsTapeClient`, `CyclingTimestamp`, `ActivityTabs`, `Tooltip`, `MobileNavDrawer` — dormant), and row affordances (`WatchlistToggle`, `WatchStar`, `MediaAttentionCell`, `MemberHeader`, `RaceIncumbentCard`, `StateFlag`, `SponsorPhoto`, `SponsorHoverName`). See `### Server / client split` for the canonical set.
...
- `/primaries` — 2026 primary + runoff calendar (HOs 91-96/107/120). Server page; `GroupTabs` (the **`races` electoral group** as of HO 173 — sub-strip Races · Primaries; lights the Races top-nav tab, not Members); the per-page `TerminalPrompt` masthead was removed in HO 185 (see `/reports`). Backed by `getUpcomingPrimaries` / `getPastPrimaries`; URL params `?chamber` / `?party` (sanitized via `sanitizePrimaryChamber` / `sanitizePrimaryParty`) + `?page` (`PAGE_SIZE = 30`). Rows render via `PrimaryCalendarRow`. Reads `primaries` + `primary_candidates` (see "Runoff tracking" + the primaries cron notes).
...
Markets ticker tape(s). **HO 178: the dashboard mounts TWO counter-scrolling tapes** (`HomeHeader`, in a `.markets-tape-block` between the masthead and the nav) — **equities** (9: SPX, NDQ, DOW, ITA, XLK, XLV, XLF, XLE, XLI) scrolling one way and **commodities/macro** (8: WTI, GOLD, SILVER, NATGAS, DXY, TNX, VIX, BTC) the other. A `group` field on `MARKET_SYMBOLS` partitions them; `MarketsTape` takes `group` + `reverse` (flips one via `animation-direction: reverse`) + `showMeta` props. **HO 185 mounted dual tapes on EVERY page; HO 187 REVERTED that — the tape is DASHBOARD-ONLY again.** Only `HomeHeader` mounts the shared **`<DualMarketsTape />`** (the equities → / commodities ← pair in a `.markets-tape-block`); `HeaderBar` no longer mounts a tape (HO 187 unmounted it as part of slimming the inner-page chrome to 4 bands; `DualMarketsTape` stays in the tree — the dashboard uses it). The no-group combined `<MarketsTape />` form is unused but left in place. The tape's `MarketsTapeClient` + the HO 183 cycling AS OF stamp (bottom tape) now live on the **dashboard only**; inner pages show a static `UPDATED … MT` in the title-bar sync instead. Feeds off `getLatestMarketTicks()` (HO 142) — **17 MarketTicks as of HO 178** (HO 168 had 8, HO 177 → 14, HO 178 added silver/natgas + re-added DXY, dropped copper). Newly-added symbols show `—` (no arrow/change) until the next trading session gives them a prior-session reference.
...
- The full `"use client"` set is 27 components (grep `^"use client"` under `components/` to refresh — minus the two hook files `useSingleOpenPanel.ts` / `use-watch-toggle.ts` that the same grep catches; this list drifted to just two for several handoffs before the HO 160 sweep, then carried an inaccurate 28-name list until the HO 161 sweep; `TopStallsList` added in HO 164, `CompetitiveRacesStrip` added in HO 166 then **dropped in HO 178** when the races strip went client→server; `CyclingTimestamp` added in HO 183; **HO 187–189 added none** — the topic-hover is CSS, the bill-row enrichment is server/column-level, the inner-chrome rework is layout; **HO 191 added none** — the stage pipeline + avatar are nested inside `BillExpandedPanel`/`SponsorHoverName`, not new islands; **HO 192 added `SponsorHoverName`**; **HO 194–200 added none** — 194 refined `SponsorHoverName`, and 195–200 (Members chrome/list/scatter/dedupe/3-column card) are all server-component, page, or query changes): filter/URL chips `StageFilter` `PartyFilter` `StateFilter` `CeremonialToggle` `SortDropdown` `SearchBox` `SearchTabs`; feed + dashboard accordions `BillRowList` `BillRow` `BillExpandedPanel` `TopStallsList`; interactive charts `DashboardBubbleChart` `PatternBubbleSVG` `StageFunnel`; chrome `MarketsTapeClient` `CyclingTimestamp` `ActivityTabs` `Tooltip` `MobileNavDrawer` (dormant); and row affordances `WatchlistToggle` `WatchStar` `MediaAttentionCell` `MemberHeader` `RaceIncumbentCard` `StateFlag` `SponsorPhoto` `SponsorHoverName`. `TopicFilter`, `SegmentedToggle`, `NewsFilters`, `Pagination`, `MemberProductivityScatter`, `PatternDrilldownPanel`, `MarketsTape`, `BreadcrumbMasthead`, `DualMarketsTape`, `GroupTabs`, and `ActiveFilterStrip` are server components despite their interactive feel — they render `<Link>`s and let the router carry state. Most client islands exist only to push/replace URL state; `WatchlistToggle` is the lone POST.
...
- **Cycling timestamps (`lib/zone-cycle.ts`, HO 183).** `formatInZone(iso, zone)` + the `useZoneCycle()` hook re-render a FIXED past instant through US zones **ET→CT→MT→PT→UTC** (4s/zone, 20s loop) — not a live clock. DST-correct via IANA `Intl.DateTimeFormat`; generic `ET/CT/MT/PT/UTC` labels (no DST-flapping EDT/EST). The active zone is **clock-derived** (`floor(Date.now()/ZONE_MS) % ZONES.length`), so the two cycling stamps — the dashboard masthead LAST SYNC (`components/CyclingTimestamp.tsx`, a client island) and the markets-tape AS OF (the bottom tape — **dashboard-only since HO 187 removed the inner-page tapes**) — stay in sync with no shared ticker. (Inner-page title-bar sync uses static `formatLastUpdated` MT, not the cycling stamp.) Reduced-motion → static MT. This is the **third named motion exception** (cursor blink + tape marquees + this).
