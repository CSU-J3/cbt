...
- **Stooq** (`https://stooq.com/q/l/`) — markets ticker (HO 142/168): SPX, WTI, the ITA/XLK/XLV sector ETFs, and GOLD (`xauusd` spot). Public CSV endpoint, no auth, 15-20 min delayed. (DXY dropped in HO 168.)
...
-- lib/markets.ts. HO 168: 8 symbols — Stooq for SPX/WTI/ITA/XLK/XLV/GOLD,
-- FRED (end-of-day) for TNX + VIX (`VIXCLS`). DXY dropped; VIX no longer
-- deferred. ~26k rows/year (8 symbols × 13 ticks/day × 252 days); history
...
- `getRace(id)` / `getRaceCandidates(raceId)` — back the `/race/[id]` hub **and the dashboard race-card drawer** (HO 166, via `GET /api/race/[id]/hub`). Both cached, tag `races`. `getRaceCandidates` orders by status precedence — `won_primary` first, then `running`, `declared`, `withdrew`, others — then name. **HO 171** seeded `race_candidates` for the 4 strip races (challengers only — the incumbent stays on the race row/card; Carter `C001103` + Collins `C001129` carry bioguides so they link to their member hubs). These are pre-primary — see `docs/backlog.md` for the post-primary refresh reminder. The `/api/revalidate?tag=races` route is the manual-flush hook; the seed scripts don't POST it — hit it after a seed run.
- `getRaceRatings(raceId)` / `getMostCompetitiveRaces(cycle, limit)` — back the rating chips on `/race/[id]` and the member-hub seat-up indicator (handoff 71). Both cached, tag `race-ratings` (separate from `races` because the rating seed refreshes quarterly on a different cadence). `getRaceRatings` returns rows sorted by `updated_at DESC`, so consumers can take `[0]` for the freshest read across sources. `getMostCompetitiveRaces` orders by `MIN(ABS(rating_score))` per race (so a single Toss Up rating from any source floats the race up), tiebreak `MAX(updated_at)`; **as of HO 163 it backs the dashboard competitive-races strip** (`CompetitiveRacesBlock`, a Senate-led chamber mix — see the `/` entry). The `/api/revalidate?tag=race-ratings` route is the manual flush hook after a re-seed.
