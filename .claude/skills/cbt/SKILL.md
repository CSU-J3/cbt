...
**Mobile nav = the inline nav (HO 156 follow-up).** There is no hamburger; the inline nav *is* the mobile nav at every width. The two header navs differ in wrap behavior (corrected in the HO 160 sweep — the prior text claimed both wrap): the **dashboard** nav `.home-header-nav` carries `flex-wrap: wrap` in `globals.css` and wraps to two rows on a 375px phone; the **HeaderBar** nav `.header-nav` (every non-dashboard page) is a Tailwind `flex items-center` with **no explicit `flex-wrap`**, so it does not wrap — it relies on the six short labels fitting. Whether `.header-nav` should also wrap on narrow widths is an open *code* question — flagged for a separate handoff (HO 160 is doc-only), not settled here.
...
| HeaderNav / home-nav | 16 / 14–16px | same | same | home-nav (`.home-header-nav`) **wraps** at all widths; HeaderBar nav (`.header-nav`) does **not** wrap (no `flex-wrap`). Drawer dormant — see above. |
