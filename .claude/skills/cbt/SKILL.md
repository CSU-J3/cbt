...
- **Primaries data debt — hidden by a UI guard, NOT fixed (HO 208).** `/primaries`' voted-block guard hides past contests with no roster AND no result, so the page looks clean while the underlying data isn't. Two distinct problems sit behind it: (1) **FALSE CONTESTS** — `senate-{ST}-2026-{D,R,open}` rows the primaries sync seeds for ALL 50 states even where no Senate seat is up in 2026 (confirmed: **IN / PA / CA Senate** — those seats are 2028/2030). The root fix is at the **sync** (don't seed senate primaries for states with no seat that cycle); the guard only hides them. (2) **REAL BUT UNROSTERED** — genuine 2026 contests whose Ballotpedia roster scrape is missing (**SD / WV Senate** — both real 2026 seats, Rounds/Capito — plus ~25 House). These should be **backfilled**, not suppressed. Don't let the guard become the permanent answer for either: the false rows need deleting at the source, the real ones need their rosters.
