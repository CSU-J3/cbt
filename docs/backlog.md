...
- **district-geo (HO 224):** Census cd119 TopoJSON + `lib/district-geo.ts` projection helper + `districtSeatId` join, all 119th-keyed. Unwired — consumed by the races modal (spec-2) and primaries modal (spec-4), not yet built. `DISPLAY_STALE_STATES` flag is opt-in, default draws 119th everywhere.
