...
- **Design-spec reconciliation — in-flight (1 building, HO 242)** (RESOLVED 2026-06-13). Design adjudicated the five unbuilt specs: three were building, three closed-no-build (struck from this ledger, tombstoned in DONE). In-flight pointers, each closed when it ships:
  - ~~(a) **Trends TIMELINE chart**~~ — **SHIPPED HO 243** (commit `76372a2`). `BillsIntroTimeline` on `/trends` (first, above `BillsTimeSeries`): calendar-axis total-introductions line, dashed year divider, partial current-month tail; new `getIntroductionsByMonth` (same universe as `getBillsByMonth`). Stayed under Patterns; COMPARE overlay dropped (no 118th intros).
  - (c) **Reports stat strip (LAWS / INTRO / MOVES) + detail reorder** — building, **HO 242**.
  - ~~(e) **NEWS SIGNAL cut**~~ — **SHIPPED HO 241** (commit `5e2c512`). `/bills?mode=news` SIGNAL row (ALL · BREAKING), `?signal=breaking`, amber rail + BREAKING pill on qualifying rows, per-scope BREAKING chip count. Aligned to the app-wide 72h breaking window, not the spec's 48h.

  *Closed-no-build (→ DONE):* (b) MOST STALE sort, (d) BillRow density redesign, PRESIDENT sub-tab repoint. *Design-index correction (unchanged):* the ◆ divergence flag **shipped on `/races` LIST in HO 222** — not deferred; spec-1→222, spec-3→223+224, metro-zoom→236/237.
