...
- **Design-spec reconciliation — five unbuilt specs** (NEW). The Design cross-ref marked these SHIPPED by name-matching older HOs; live code (verified Jun 12) says otherwise:
  - (a) **Trends promotion + TIMELINE chart** — `/trends` still renders the HO 66/76 charts; Trends is a Patterns sub-tab, not promoted.
  - (b) **Stale → MOST STALE bills sort + "301"** — `/stale` is standalone; sort is `action|introduced` only, no MOST STALE.
  - (c) **Reports index stat strip (LAWS / INTRO / MOVES) + detail reorder** — absent.
  - (d) **BillRow collapsed-row density redesign + HO 191 pipeline tighten** — no 88px party-railed grid.
  - (e) **NEWS SIGNAL cut** — no `?signal` param anywhere.

  All five are weeks stale: re-validate against live state (post-230 nav, post-184 routes) or reroute to Design before scoping. *Close:* each scoped, rerouted, or tombstoned. *Tombstoned inline:* **PRESIDENT sub-tab repoint** — premise dead; `/president` is the HO 151 redirect alias, the dedicated desk page was deleted in HO 151/154. Re-open only as a fresh design question. *Design-index correction:* the ◆ divergence flag **shipped on `/races` LIST in HO 222** — not deferred; spec-1→222, spec-3→223+224, metro-zoom→236/237.
