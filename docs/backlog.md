...
- **district-geo (HO 224):** Census cd119 TopoJSON + `lib/district-geo.ts` projection helper + `districtSeatId` join, all 119th-keyed. **Wired into the `/races` district modal (HO 225)** via `GET /api/races/district-geo/[state]`; the **primaries modal (spec-4) is HO 226, not yet built.** `DISPLAY_STALE_STATES` flag is opt-in, default draws 119th everywhere (the modal does not yet act on it).
