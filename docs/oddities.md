...
**Addendum, HO 746 (2026-09-24).** The Senate half of this entry no longer describes the code. `1faaac2` replaced its `MAX(roll_call)` watermark with the session's stored roll set (`getStoredRolls`): a menu roll is now skipped only if that exact roll is stored. That answers this entry's question's complement, *what re-visits a row that was never written?*, to which the watermark's answer was "nothing". A roll that failed before its `votes` row was written was stranded as soon as a later one landed (HO 744's backlog line; HO 745 measured none stranded; HO 746's leg planted one and the new rule filled it). The entry's own point stands, unchanged: the stored-set skip still never re-fetches a stored row, so a row written *wrong*, an empty roster, is still the HO 567 heal pass's. The House half, `MAX(vote_date)`, is untouched.
...
**Third sighting, HO 739 (2026-09-19), and it is the same family with a different metacharacter: backticks, eaten as command substitution.** A SKILL sentence was rewritten with `python -c "…"` inside double quotes, and every backticked token in the replacement text — `CRON_SECRET`, `302`, `vercel.com/sso-api`, `curl -f`, `JSON.parse`, `playwright.config.ts` — was executed by bash and substituted with its (empty) output before Python ever saw the string. The prose landed with holes in it: *"to wall the pre-rotation deployments carrying the burned , and the prediction held"*, *"the wall answers  to ,  does not fail on a 3xx"*. **Every surface reading was clean.** The script printed its own `rewritten` success line; `git diff --numstat` read **1+/1−**, exactly the one-line in-place rewrite that was intended; `check:design-citations` passed; and `git ls-files --eol` was clean. A numstat cannot see inside a line, and a script that reports success reports the success of what it was handed, not of what was meant. **What caught it was reading the replaced text back out of the file**, which is the only instrument that could have. **The check that now settles it** is a count taken on the passage itself — 1905 characters with **32 backticks, an even number** — because an eaten pair leaves an odd count or a missing token, and both are visible without re-reading the prose. **The rule generalises past both sightings: a value that crosses this box's shell loses its metacharacters** — backslashes to a heredoc (HO 678, HO 700), backticks to a double-quoted `-c` string here — so text carrying either is written to a FILE with the Write tool and executed from there, never passed inline. Related: the `git commit -m` backtick entry near the head of this file is the same mechanism on the same tool, filed before it had a family.
...
**Boundary 3 — "the name the handoff assigned".** The one that cost the most, and
the only one that produced a false record rather than a missing one. That
companion was first dispositioned `never-existed` on the evidence *zero commits
touch that path* — which answers whether a **name** was ever committed, not
whether the **artefact** exists. It exists: `docs/design/dashboard-layout-target.html`,
`<title>MOCK 591 · dashboard layout reference</title>` — **renamed** at HO 604 C0
from `mock-591-dashboard-layout.html` (`604-type-scale.md:62`, tracked archive),
under that handoff's stated policy of descriptive names without HO numbers
(`:64`). Its companion is the same story one level up: the C1-C8 visual dictionary
recorded as *did not land* is `docs/design/mock-591-conventions.html`, eight
`DEFECT`/`FIXED` pairs. **It was assigned the same rename and never got it**, so it
sat in `docs/design/` under its original name — and the timeline is the whole
story: C0 landed **01:42Z**, the file was saved **02:30Z**, and the HO 605 sweep at
**02:46Z** looked for it *"in the project root"* (`605-doc-sweep-603-604.md:17`)
and recorded it never landed, **sixteen minutes after it arrived in the directory
that sweep was writing about.**

**So HO 696's first cut reproduced HO 604's mistake inside the fix for it — and
then got the mechanism wrong a second time**, calling the assigned name HO 591's
and the companion's a download artefact, when the archive says both were HO 604 C0
rename targets. Two corrections deep on one finding, each from reading a source the
previous pass had not. The first instance of the shape is older still:
`welcome-670.html` (HO 670), where the handoff's expected name and the signed-off
file's name differed and the roadmap sentence had to correct itself.

**THE INSTRUMENT FOR "THIS ARTEFACT IS MISSING" IS THE FILE'S OWN `<title>`, READ
FROM DISK — never a filename grep; and the instrument for WHY it is missing is
the handoff that named it, read from the tracked archive.** A filename is what somebody called it in
prose; the title is what the artefact says it is. Where the two can disagree, only
...
## A synchronous error-time DOM read is not an earlier settled read — it is a read of a different thing (HO 700, 2026-09-07)

The HO 687 `#418` dump aligns the served SSR bytes against the DOM **after settle**, so its `addedInDom` is a superset of the hydration mismatch and six weeks of prod fires stayed unattributed (`backlog:50`, opened 2026-07-29 at the first observed run). The obvious narrowing — capture the DOM at the moment of the fire — was built: an `addInitScript` `window` `error` listener storing `document.documentElement.outerHTML` inside its own handler, installed before any page script, on the premise that React 19's `reportError` dispatches synchronously in the commit's own task and therefore lands **before** any `useEffect` output. The premise about `useEffect` is true and irrelevant. **Measured on 6 of 6 fires against a tree that fires on demand (`ho697-prefix-clock`): the listener caught every event — `fireCaught` true, `fireCount` 1, `fire-t` 190-1215 ms — and the DOM it captured was element-identical to the SSR bytes, `+0/-0`, including a tape route whose settled read was `+127/-36`.** React dispatches the recoverable-hydration error when it **detects** the mismatch, before it **commits** the client re-render that fixes it. So the fire-time DOM contains no mismatch by construction: it is not a cleaner version of the settled read, it is the *pre-recovery* document, and the one thing it can never contain is the thing the instrument was built to see.

**The trap worth carrying forward is the diagnosis, not the mechanism.** The handoff anticipated an empty fire-time set and pre-assigned it a cause — *the listener is missing the event, installed too late or matching the wrong message*. Both readings produce `fire-added=[none]`, and only the *presence of the capture metadata* separates them; without `fireCaught`/`fireCount`/`fire-t` in the dump, a correct listener reading a pre-recovery DOM is indistinguishable from a broken listener reading nothing, and the arm would have been abandoned for the wrong reason — or worse, "fixed" by moving an installation point that was already correct. **When a probe's null result has a pre-assigned cause, record the evidence that would separate that cause from its neighbours in the same dump.** One boolean did it here.

**What the same run measured about where the window actually is.** Three extra capture points, temporary: microtask, macrotask, and one frame (`requestAnimationFrame` + `setTimeout 0`). Microtask and macrotask both still read `+0/-0`. The frame read caught it — but on `committee-detail` it landed *after* the recovery render (`+3/-0`, naming the two clock spans and the announcer) and on the tape route `dashboard-v2` it landed *before* it (`+1/-0`, the announcer alone, against a settled `+127/-36`). The window between the recovery commit and the passive-effect flush is therefore **real** — a tape route inside it shows the announcer and none of the ~127 tape insertions — but **a fixed time offset samples the wrong side of it depending on the route**, so the instrument that can use this window has to be event-driven (the first mutation batch after the fire), never a deferral tuned to a number that worked once.

**And the second half of the sentence this entry supersedes.** The HO 697 counting-arc closer ends *"the instrument that can [name the node] is one that reads the DOM at fire time, or a development build."* Both halves are now measured false: fire-time reads the pre-recovery DOM, and a development build **does not fire at all** — `next dev` of both trees read 0 fires in 360 navigations in the same hour production-mode crawls of those same trees read 1 · 1 and 0 · 2.

## A backtick in a comment inside a template literal, and a grep that filtered the crash out (HO 702, 2026-09-08)

The intervention harness stopped transforming and every mode "ran" in one to two seconds. The output, filtered through `grep -E 'fired=[1-9]|MODE='`, read: `control end`, `t2 end`, `t2t1 end`, no fires — **a clean empty window, indistinguishable from a window in which nothing fired.** The cause was a comment *inside* a template literal that wrote `` `first-post-t` `` and `` `self.webpackChunk_N_E` `` in backticks; a backtick terminates the literal, so the file no longer parsed. The same bug had already been sitting in `e2e/smoke.spec.ts` from an earlier edit and detonated one insertion later.

**Two gates should have caught it and neither could.** `npx tsc --noEmit` returned **0**, because `tsconfig.json` excludes `scripts/diagnostic/scratch` and omits `e2e/**` from `include` — the file was never in the program (filed as an OPEN LOOP). And the grep filter, written to keep the output readable, removed the stack trace along with the noise. **A filter applied to a command's output is part of the instrument**, and one that can only match success lines converts a crash into a silent zero.

The remedies are both cheap and both are in the tree: the constraint is stated where it can be violated —

> NO BACKTICKS IN HERE: this comment lives inside a template literal and a backtick terminates it. tsconfig excludes scripts/diagnostic/scratch, so tsc cannot catch it either — the runtime is the only gate.

— and the corrected window writes its **full, unfiltered** output to a file and greps that file afterwards, so a crash cannot hide as a zero a second time. **This is the fourth instrument in one HO that could not fail:** a patch that never reached the bundle (managed path, byte-identical chunk hash, build green), a site tag in the text-flag slot reporting every structural mismatch as `text`, a widened message that silently disabled the dump it depended on, and this. Every one was caught by a control or a marker rather than by reading the code.


## Three instrument failures in one HO, all in the same discriminator (HO 704, 2026-09-08)

HO 704 added one line — `<Suspense fallback={null}>` around `{children}` in the root layout — and needed to tell a fixed build from an unfixed one, and a cheap boundary from an expensive one. **Three separate instruments answered confidently and wrongly before any of them answered correctly**, and all three failed the same way: each read a proxy for the thing rather than the thing.

**1. Marker presence is not a fixed/unfixed discriminator.** The obvious check — "the fixed build emits Suspense boundary markers, the unfixed one doesn't" — reads green on the **unfixed** tree. Next 15.5.15 already emits **two** `<!--$--><!--/$-->` pairs on every route: one inside a `<div hidden>` immediately after `<body>`, one after the app content. Both are **empty** — open marker immediately followed by its close, wrapping nothing — because they are Next's own metadata and route-announcer placeholders. The corrected discriminator counts pairs with **content between them**: 0 before, 1 after. The accident is a gift: those two empty pairs are the cleanest possible confirmation of the premise the whole HO rested on, that **no boundary sat between the root and the page segment**, measured off the served bytes rather than argued from a grep for `<Suspense>`.

**2. `<!--$?-->` — React's pending-boundary marker — was unknown to the parser, so the build that HAD a boundary read as having none.** React emits four markers, not two: `<!--$-->` opens a **resolved** boundary, `<!--$?-->` a **pending** one whose content is deferred to a stream, `<!--$!-->` an errored one, and `<!--/$-->` closes any of them. A regex written as `/<!--(\/?)\$-->/` matches the first and the last and silently misses the middle two. The fixed build therefore reported **NO-SEGMENT-BOUNDARY** — the exact string the unfixed build reports — while its raw HTML plainly read `<!--$?--><template id="B:0"></template><!--/$-->`. **The failure and the success printed identical verdicts on opposite builds**, which is this file's recurring subject arriving through a regex. It was caught only by dumping the raw bytes around every marker instead of trusting the count.

**3. A transfer floor computed from DECODED bytes over a COMPRESSED wire, which would have falsified a correct mechanism for an instrument reason.** The question was whether a server-suspending boundary makes the page segment atomic — content painting at last byte rather than progressively — so the test compares FCP against the time the document takes to arrive. `response.body()` returns the **decoded** body; `next start` compresses the document ~5×. Dividing decoded bytes by the emulated throughput produced a **2680 ms** "floor" on `/` against a measured **656 ms** FCP, i.e. *content painted long before the document could possibly have finished arriving* — a clean, quantitative, entirely wrong refutation. The real floor, from `request().sizes().responseBodySize`, is **551 ms**, and 656 ms is floor-plus-swap: the mechanism confirmed, by the same numbers that appeared to kill it. **The architect's predicted magnitudes inherited the identical error** (~2.7 s on `/`, ~13 s on `/members`) because the byte figures handed up the relay were decoded sizes reported without their encoding. Both tables were discarded and re-run.

**The rule the third one leaves: a byte count without its encoding is a claim, not a measurement.** Say `wireBytes` or `decodedBytes`, never `bytes`, anywhere a number is going to be divided by a bandwidth or compared against a transfer time — and that applies to a figure travelling up a relay just as much as to one in a script, since the second reader cannot see which one it was. `scripts/diagnostic/throttled-fcp-704.ts` names `request().sizes()` as its byte source in its own header for exactly this reason.

**What ties the three together, and why they are one entry rather than three.** Each instrument was *checking the right thing* and *reading the wrong artifact for it*: markers instead of non-empty markers, one marker vocabulary instead of four, decoded size instead of wire size. None was caught by review; **all three were caught by dumping the underlying artifact and looking at it** — the raw HTML around each marker, and the response's own size record. The cheap habit that would have caught all three up front is the one this file keeps arriving at from new directions: before trusting a derived count, print the thing it was derived from, once.

## A boundary that removes an error and pays for it in paint (HO 704, 2026-09-08)

The HO 702 cause was confirmed and its remedy worked: `<Suspense fallback={null}>` around `{children}` took the `#418` fires from **6 of 36 to 0 of 36** under the throttle lever, in one interleaved window, while `HYD-BEFORE-TAIL` stayed at **36 of 36** — the race still fired on every hit and only the error stopped. It was declined anyway, and the reason is worth keeping because it is invisible on the machine where the fix gets written.

**A Suspense boundary that suspends on the SERVER makes what it encloses atomic.** React flushes the shell immediately and streams the segment into `<div hidden id="S:0">`, patched into place by a `$RC` call after its closing tag — so the enclosed content paints at **last byte** instead of progressively as bytes arrive. Measured against the encoded document on a local production build: unfixed FP and FCP are **identical** on all four cells with a **0 ms** blank window, and unfixed FCP is nearly flat across document size and throughput (272 / 300 / 184 / 316 ms) because paint tracks arrival. Fixed FCP tracks the **completed transfer**: floor 551 → FCP 656, floor 2394 → FCP **2880**. Worst cell, `/members` at 1500 kbit/s: **2.56 seconds of painted-but-empty page** where the unfixed build had content at 300 ms.

**Localhost reports this boundary as cheap and it is not.** Unthrottled, the same pair reads +256 ms and +388 ms — parse-and-swap alone, with transfer removed from the equation. That is the **floor** of the cost, not the cost, and it is what a developer measures by default. The cost scales with document size and inversely with bandwidth, so the reader who pays most is the one on the worst connection, who is also the one no local run represents.

**The generalisable half, now a standing rule in SKILL:** no server-suspending boundary above the page segment without a designed fallback **and** the throttled FCP reading in the paste. And the narrower trap for anyone reaching for the same fix: **FP improving is not evidence of anything** — an empty shell painting early improves FP while FCP regresses, so a boundary change is read on the FP/FCP *split*, never on FP alone. The declined tree is preserved at tag `ho704-root-suspense`.

## Two homes for one default, and the second one silently wins (HO 703, 2026-09-08)

A default was promoted in the file that owns it and left behind in the file that calls it, and the caller's copy is the one that ran. `e2e/narrow.spec.ts` took `NARROW_WIDTHS`'s default from `430` to `430,390` after HO 703 measured 390 clean on prod. The workflow that invokes it kept `NARROW_WIDTHS: ${{ inputs.narrow_widths || '430' }}` — and **`inputs` does not exist on `deployment_status` or `schedule`**, so the fallback fired on every non-dispatch trigger and pinned the deploy-blocking Production run to one width. The `workflow_dispatch` input's own `default: '430'` said the same thing a third time.

**The reason it survived a careful arc is that no reading could reach it.** The HO 703 gates were real ones: the new Preview job was seen red and green on the day it landed, each of its three controls was fired separably with its own run id, and the 390 census came from tabulating all 72 `[narrow …]` lines of a dispatch rather than trusting the run conclusion. Every one of those paths sets `NARROW_WIDTHS` explicitly (the dispatch) or sets no env at all (the Preview job, which reads the spec default correctly). **The single path that reads the workflow's fallback is a Production `deployment_status` run, which by construction cannot exist until after the fast-forward the review is deciding.** So the value was verified everywhere it could be verified, and shipped wrong on the only trigger nobody could reach.

**Three documents and one comment had already been written against the intended behaviour**, which is what makes this worth a note rather than a typo: SKILL said *"IT READS TWO WIDTHS SINCE HO 703"*, `backlog:86` closed on *default `430,390`*, the roadmap block described the input as defaulting to `430` as if that were the safety, and the Preview job's own comment said *"the Preview gate and the daily cannot drift"* — the drift the smoke job was forcing. Prose agreeing with prose is not corroboration when all of it is downstream of one unread line.

**The fix is one home, not a smarter fallback**: the env passes `${{ inputs.narrow_widths }}` bare, the dispatch input defaults to empty, and `e2e/narrow.spec.ts:118` holds the only literal width the narrow gate reads; `odds-off.spec.ts:41` and `fit-finish.spec.ts:1012` carry their own independent viewport sets, which this invariant does not govern. **And the fix has a tail worth knowing before anyone repeats it** — a bare GitHub Actions pass-through of an absent input is the **empty string**, not undefined, so a `??` default does not fire on it. Under the unfixed spec, `NARROW_WIDTHS=''` exits 1 with `NARROW_WIDTHS is malformed: ""`: removing the literal without also moving `??` to `||` would have turned every scheduled and deploy-blocking run red instead of green-on-one-width. The malformed guard is unaffected — `430,39O`, `,` and a whitespace-only value each still exit 1 — because only a wholly empty value now falls back.

**The general form.** When a default moves, grep for its literal rather than for its name: the name is in every file that discusses it and the literal is in the one that overrides it. And when a value is only exercised on a trigger that cannot run before the merge, say so in the paste as an unmeasured path rather than letting the measured ones stand in for it.

## The React that runs is not the React in `package.json` (HO 705, 2026-09-09)

**The App Router executes Next's vendored canary, not the `react-dom` the lockfile pins.** Next aliases `react` and `react-dom` to `next/dist/compiled/react-dom` on server and client alike, so CBT at `next@15.5.15` runs `19.2.0-canary-0bdb9206-20250818` while `node_modules/react-dom` sits at `19.2.5` and is never loaded by a single app route. Two consequences that are not obvious from either file: a `react-dom` bump in `package.json` is **inert** for anything in the reconciler, and the whole `15.5.x` line is inert too — 15.5.15 and 15.5.25 ship a **byte-identical** hydration client (`react-dom-client.production.js`, 15,839 lines, sha256 `9bcaacd9…053e`).

**The costly half is that this produced a "correction" that corrected the right anchors into the wrong ones.** HO 704's handoff cited `resetHydrationState` at `:2879` and `retryDehydratedSuspenseBoundary` at `:11749`. STEP 0 dutifully re-derived them at HEAD, found `:2875` and `:11872`, and filed the originals as stale — but the re-derivation read `node_modules/react-dom`, the file the App Router never executes, and the handoff's originals had been read off the **vendored** file and were right all along. A re-derivation is only as good as the file it re-derives against, and "I checked it at HEAD" does not name a file.

**The discriminator is what the browser asked for, never a directory grep.** `.next/static/chunks` genuinely *does* contain the string `19.2.5` — in `framework-*.js`, the **Pages-router** chunk that `0 of 58` app routes reference and no app document ever requests. A grep over the chunk directory therefore reports the inert version as present and reads as though the premise were refuted. The two readings that answer the question are `app-build-manifest.json` (which chunks a route loads) and the served document's own `<script src>` list. Both were taken; they agree; the grep does not.

**And when you need to know whether a specific upstream fix is in a bundle, measure the behaviour, not the symbol.** `react#35494` adds `popHydrationStateOnInterruptedWork`, but every published build **inlines** it, so grepping the name reads `0` on fixed and unfixed bundles alike — a gate that cannot fail, run six times before the zero looked suspicious rather than clean. The discriminator that works is the shape it leaves behind: a `hydrationParentFiber` restore inside `replayBeginWork`'s `case 5`, present in `19.3.0-canary-cbb046ab-20260731` and absent from `19.2.0-canary-0bdb9206-20250818`, `19.2.5` and `19.2.8`.


## An instrument whose precondition is the bundler, and a zero that meant "unread" (HO 705, 2026-09-09)

`scripts/diagnostic/seam-intervene-702.ts` stamps `hyd-start` by trapping the assignment of `self.webpackChunk_N_E` — the global the first app chunk defines as it begins executing. That is a **webpack** artifact. **Turbopack never defines it**, and Next 16 builds with Turbopack by default: the global appears in **78** chunks of a `15.5.15` build and **0** of a Turbopack `16.3.4` build.

So the first HO 705 window read `FIRES=0` on both Next 16 legs — and `HYD-BEFORE-TAIL 0/36`, `hyd-after-tail 0/36`, **`n/a ×36`**. The fire counter worked; the comparator that establishes *the race even occurred* did not, because one of its two operands could never be stamped. **A `t2` leg of 0 on a build where the race is unmeasurable is the section's own subject in its purest form**: the fixed build reads zero whether or not the condition it is supposed to survive ever arose, so the headline number and a completely broken run are the same output. The only thing separating them was a comparator column that a reader in a hurry would have skimmed past on the way to `FIRES=0`.

**The fix was to restore the precondition rather than reinterpret the zero.** `next build --webpack` (Next 16.3.4 still ships the flag) put the global back in 82 chunks, left the vendored React untouched — the served document still requested only `19.3.0-canary-cbb046ab-20260731` chunks and never `framework-*.js` — and, as a bonus, put both legs on the same bundler, removing bundler as a confound instead of leaving it as one. The window was then re-run whole rather than patched leg-by-leg, and the void one is kept beside it at `705-artifacts/window-1-turbopack-unread.log`. **The general form: before trusting a green from a ported instrument, check that the instrument's own preconditions survived the port** — and prefer an instrument that reports `n/a` over one that reports `0`, because this one told the truth and was nearly read as a pass anyway.


## `.env.production.local` with all-empty keys silently shadows `.env` (HO 705, 2026-09-09)

`next start` loads `.env.production.local` over `.env`, and the copy in the CBT checkout defines `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN`, `CONGRESS_API_KEY`, `GEMINI_API_KEY`, `AUTH_SECRET` and four more as **empty strings**. An empty value is a value, so it wins, and a local production server 500s on every DB-backed route while `.env` sits right there fully populated. Nothing announces the shadowing; the file's remaining keys (`VERCEL*`, `TURBO*`, an OIDC token) look like the plausible reason it exists.

**This is the stale-`.next` class one file over** — a local artifact that makes a correct tree behave like a broken one, where the fix is to remove the artifact rather than to debug the tree. HO 705 deleted it in both throwaway worktrees, which is why the two production servers came up clean with no `source .env` incantation.

**The main checkout's copy was deliberately left in place, and the reason is recorded rather than assumed:** HO 705 is docs-only, the checkout is shared with concurrent sessions, and the file's one live-looking value (`VERCEL_OIDC_TOKEN`) has never been audited for what reads it. So it stays, and the next session to run a local production server should delete it — or `export` the real values, which beats it because process env outranks every `.env` file. **HO 706 will run local production servers and is the natural place to settle it.** **RULED AND DONE (HO 706, 2026-09-09): it is deleted from the main checkout.** Both HO 705 worktrees had already run production servers without it, its empty keys are what shadow `.env`, and the one value that looked like a reason to keep it — `VERCEL_OIDC_TOKEN` — is regenerated by `vercel env pull` whenever the CLI needs it. The HO 705 block recorded the opposite decision; this supersedes it.

## Two production servers, two cache generations, and a pixel diff that reads as a regression (HO 706, 2026-09-09)

**A bundler change is render-touching, so the 16 build was pixel-diffed against the 15 build across the whole crawl set at three viewport configs — 108 captures, Playwright's own comparator.** 103 passed. Of the 5 failures, three were `/bills` at **all three viewports**, and that is the entry: a route-consistent, viewport-independent failure is the exact signature of a real render regression, and it is what a stale cache produces too.

**The obvious control did not separate them.** Live-data drift was the first hypothesis, so the failures were re-captured back-to-back seconds apart. Two of them (`home @2560`, `home-stage-introduced @1440-reduced`) passed and were disposed of. **`/bills` failed again**, which is precisely the evidence that is supposed to rule drift out — no time had passed, so the difference had to be the build. It was not.

**The two servers were separate processes holding separate `unstable_cache` state.** The one started earlier was serving an older generation. Both returned 25 rows; the newer build's list was **shifted by exactly 7** — seven bills the sync cron had written in between, visible to the fresh cache and not to the warm one. Nothing about that is bundler-related, and no number of pixel re-runs could have said so, because both servers were stably wrong relative to each other.

**What settled it was leaving the pixels entirely.** Diffing the *served HTML* — `class="row-title"` text, 25 per side — showed the shift immediately and named it as content rather than rendering. Then `POST /api/revalidate?tag=bills` against **both** servers, re-diff: `IDENTICAL`. Re-capture: 3 of 3 pass. Two rules, both now in `e2e/bundler-captures-706.spec.ts`'s header: **flush every tag the routes under test read, on both servers, before any pixel is read**, and **when a capture fails, diff the served HTML before the PNG** — it is cheaper and it answers *"same content?"* directly, where a pixel diff only answers it by implication.

**The measured drift rate exists and is not zero, which is what makes a small failure count readable rather than alarming.** Re-running the n15 baseline against the **n15 build itself** produced 1 failure in 108 (`home-stage-president @2560`), which passed back-to-back. That control is the reason "5 failed" could be triaged instead of panicked over — without it, the honest reading of 5/108 is unknown, and the temptation is to raise `maxDiffPixelRatio` until it goes away, which would have hidden the cache finding along with the noise.

## An instrument that names a property it does not measure (HO 708, 2026-09-09)

**HO 706's capture gate ran three viewport configs, and the third one emulated nothing.** `e2e/bundler-captures-706.spec.ts:67` set `reducedMotion` inside `test.use()`. That key belongs to `BrowserContextOptions`, not `TestOptions` — Playwright's runner treats an unrecognised `use()` key as a **fixture nothing consumes**, so the spec loaded without a word, `--list` exited 0 and enumerated 36 `@1440-reduced` tests, and every one of them ran as a plain 1440 capture with a longer filename.

**Every downstream signal agreed with the honest version, by construction.** The filenames carried `-reduced`. The test titles carried `@1440-reduced`. The counts were right — 36 per role, 108 per pass. The comparison passed. And it passed *because* it was the 1440 comparison run a second time against 1440 baselines, which is exactly what a correct reduced-motion pass would also look like if the layout happened not to change. **The one signal that could have separated them was suppressed by an unrelated correct decision:** the comparator ran `animations: "disabled"` on every capture, so the frozen-animation half of reduced motion was already gone from both sides, and what the third config would uniquely have added was the `prefers-reduced-motion` **layout** — the HO 670 mask class — on 36 routes, across a bundler change.

**The fastest instrument in the repo names it in under a second, and was the one not pointed at the file.** `tsc` reports `TS2353: 'reducedMotion' does not exist in type 'Fixtures<…>'` immediately — but `e2e/**` is outside `tsconfig.json`'s `include` (`backlog:14`), so `npm run typecheck` had never read that file, and neither had CI, whose own comment says `e2e/**` is *"covered by typecheck"*. A gap in what the type checker reads is not only a typing gap: **it is a place where instruments live whose green has never been checked by anything but themselves.**
...
**Sighted again at HO 736, in a capture harness, with `\s` instead of `\d` — and the damage is worse with `s` than with `d` because the survivor is a letter that occurs in ordinary prose.** The page body's normaliser read `.replace(/\s+/g, ' ')` inside the template literal, so the browser compiled `/s+/g` and every run of the letter **s** became a space: the AK roster read back as `David Le lie`, the news line as `No recent new  linked to thi  race'  incumbent`, and `Last verified` as `La t verified`. Unlike HO 712 this one is **not** silent — the output is visibly mangled to anyone reading it — but it is only loud where the replaced character is common, and `\d`, `\w`, `\b` would each have failed quietly on the same line. The fix is the same and so is the rule: `\\s` at the injection site, with a comment, or a character class that needs no backslash at all. **The compounding fact worth carrying: this is the second distinct trap living in that one string** — the body is a template literal *because* of HO 670/675/687's `keepNames` `__name` rewrite, and taking that remedy hands you this one. A string body buys immunity from the transform and inherits the escape rules; neither hazard is visible in the file.
...
## A conditional element sits outside every gate until content creates it (HO 716, Sep 2026)

`e2e/narrow.spec.ts` read `home` at 390 as **`over=0` at 23:06Z** and **`over=3` at 00:03Z**. Same gate, same viewport, same CSS, and — across the HO 714 fast-forward that sits between the two readings — code whose diff contains no `weekly-band` token at all. Nothing about the layout changed. The week's enacted bill count changed, from **0 to 4**.

`WeeklyBand` renders its id chips behind `enactedShown.length > 0`. At zero the `.weekly-band-ids` span **does not exist**, so there is no box to overflow and the gate reads a clean zero. At four it exists, holding `· HR 5366 HR 1276 HR 2069 +1`, and the latent defect underneath it — an inline-flex that cannot wrap inside a `nowrap` ancestor — finally has something to be wrong about.

**The green was the element's absence, not the layout's soundness.** That is this project's own gate principle (*before running a check, say what it reads if the work was never done*) arriving from a direction the principle as written does not cover: not an instrument misread, not a check that cannot fail, but a check that is **correct, sensitive, and simply not pointed at anything yet**. Every instance in method.md's Gates section is about the instrument; this one is about the subject failing to turn up.

Two consequences worth carrying:

**A narrow-gate green over a conditionally-rendered surface is a statement about today's data, not about the surface.** The dashboard is full of these — `.abw` (HO 714's own subject, three states now), `ActiveFilterStrip`, the `+N` chip itself — and each is measured only on the days its content exists. The worst case is unmeasured by construction.

**And the failure surfaces at the worst possible moment, which is why it reads as a regression.** Content crosses the threshold on its own schedule, so the red lands on whichever deploy happens to follow it. HO 714's Production run was the first to see it and looked, for the length of one paste, exactly like a change that had broken `/` — the diff had to be read to rule it out. The HO 683 reasoning about the smoke crawler's overflow alarm anticipated precisely this (*the class is data-armed, so blocking on it would red an unrelated push for a defect its author neither caused nor can fix*) and the narrow gate, which **is** deploy-blocking, has no such carve — correctly, since a real overflow must block, but it means the next data-armed red will wear the same disguise.

The remedy is not to weaken the gate. It is to stop the content being the variable: a fixture state that renders each conditional band at max content would make the reading content-independent, and is filed on the backlog as such.

**And it caught the VERIFICATION, not just the detection — within the same handoff, an hour after this entry was written.** HO 716's Preview `narrow-preview` run (`34661908346` on `c4c90e4`) came back **74 passed**, all eight formerly-red routes `over=0` at 390 and 430. The handoff called that reading "the measurement". It was worth nothing: the Preview deployment's Data Cache held a pre-flip generation serving **`● 0 enacted ▼1`, 312 bytes, zero chips** — verified by fetching the preview's own `/` and reading the segment — so the ids box did not exist there either and the fix was never exercised. The fixed CSS *was* deployed (`.weekly-band-seg{white-space:normal}` and `.wb-metric{cursor:help;white-space:nowrap}`, read out of the served chunk), which is what makes the false green so convincing: **every other thing you would check was correct.** A gate green, the right SHA, the right bytes in the stylesheet, and no exercise of the thing under test.
...
## The index lists a file that does not exist (HO 717, Sep 2026)

HO 717 made the House Committee Repository's event page (`docs.house.gov/Committee/Calendar/ByEvent.aspx?EventID={id}`) the target of the recorded-vote link, because it answers 200 and lists every file the meeting carries. For Appropriations event 119200 that listing includes **`HMKP-119-AP23-20260417-SD001.pdf`** under Support Documents — and that file answers **HTTP 200 with an HTML "Not Found" page on both `docs.house.gov` and `www.congress.gov`** (HO 715 row 2). Congress.gov's `meetingDocuments[]` names it too. Two indexes agree, and the object neither can serve is the one they agree about.

The link is still honest: it opens the committee's own listing, which is the record as the committee filed it. What it cannot promise is that every file on that listing opens. The count on the row, `n`, is the number of listed recorded-vote documents, and a dead listed file counts. So the SKILL entry says it in words — *a listed file is not a guaranteed file* — rather than implying that `n` links lead to `n` PDFs. A fetcher that wants the files themselves has to check each body's `%PDF-` magic; the listing's existence is not evidence, and neither is a 200.

---

## A production build shipped CSS that omitted a `globals.css` edit, and the chunk hash did not change (HO 717, Sep 2026)

HO 717 added a block of `.hearing-votes` rules to `app/globals.css`, edited at 14:22:53. `npm run build` exited 0 (`.next/BUILD_ID` at 14:25:35), `npm run typecheck` exited 0, and the served page carried the new markup (`a.hearing-votes`, two of them, from the JSX). **The served stylesheet contained zero `hearing-votes` tokens.** Neighbouring rules from the same file (`a.hearing-watch:hover`, `.hearing-panel`) were present; no `:has(` survived anywhere. The first sign was a measurement, not a look: the link computed `display: block` instead of the rule's `flex`, spanning column 1 at 1,329 px, and at 390 the collapsed row's title was squeezed to 0 px by a full label the media query should have hidden.

A second, warm build (9 s) produced **the same CSS chunk hash, `14bkpmxy3rm5z`, still without the rules** — so the source edit was not being read into the chunk, and rebuilding did not help. Deleting `.next` (309 MB, including its cache) and building cold produced a new chunk, `3d32g0h7yrdoz.css`, with nine `hearing-votes` tokens. **It did not reproduce on demand:** a later CSS edit, built warm, was picked up correctly (new chunk `2f96byqize6w_.css` carrying the new `align-items: flex-start`). Mechanism unknown; recorded as observed, not explained.

The part worth carrying is the instrument. Build exit 0, typecheck exit 0, HTTP 200 and the new markup in the DOM are all the same bytes whether or not the CSS shipped — the "stale `.next` serves 200 with 404'd CSS" family, one step subtler, because here the stylesheet loaded fine and simply lacked the rules. What caught it was **grepping the served stylesheet for a token the change adds** and reading a computed style back. Do both before any capture is trusted, and after a Vercel build as well as a local one: a build cache is a build cache.

---

## A category invented for a fact that belonged to a person (HO 718, Sep 2026)

HO 710 shipped the 2028 seat outlook as tiers of one status column: OPEN, LIKELY, TBD. HO 718 took nine chat messages and eight mocks to get rid of them. Each message removed one abstraction the tier structure had needed. *"What does signals on file mean"* found a heading that was the `open_signal` column name, and it was **false for two of the three rows under it**: Moody and Husted had signalled nothing; the calendar had. *"Why do we have special election in 2026 in 2028"* found a heading that named the 2026 event in a 2028 report. *"Do we really need it then?"* and *"could we just put that info next to the names"* dissolved TBD and LIKELY into qualifiers on the incumbent's name. The facts had been about people from the start. The tiers were a category CBT invented so that a column could hold them, and every later label (*status known*, *holder*, *statement on file*) described the category rather than the person.

**The test that would have caught it at v1:** is the heading true of every row it sits over, in words a Ballotpedia reader already has? *Signals on file* fails the first half on two of three rows, and *TBD* fails the second half on sight. SKILL now states this as the label rule for the surface.

**The same arc had one factual slip in the ruling record.** The v8 mock named Johnson's source `nbc15.com`. The seed URL (`data/races-seed.json`) is on `www.wmtv15news.com`, and the render has always derived the host from the URL. The host was typed from memory into a mock that serves as the ruling record. STEP 0 caught it by reading the seed row before anything was built. A mock can be exact about layout and still be wrong about data, so any string it shows that the render derives gets read against its source before it lands anywhere.

---

## A byte-identical claim measured against a rotating clock: same SHA, different bytes (HO 718, Sep 2026)

HO 718's STEP 0 took DOM baselines under the HO 700 strip floor (route announcer and `.micro-tag` removed), then repeated the capture seconds later as a control. **At one SHA, three of four routes differed in innerText.** The only difference was the masthead: `10:27 AM MT` against `11:27 AM CT`. `CyclingTimestamp` (HO 183) rotates the LAST SYNC zone every four seconds, and the floor, written to catch the announcer, has nothing to key on in a text node. Under the old floor a +0/−0 claim could have failed on code that changed nothing, or two captures could have happened to land on the same zone and passed by luck. Neither reading would have told you which had happened.

The remedy was already the repo's reading discipline for layout (HO 708): **take the capture with `reducedMotion: "reduce"`**, which pins the zone to MT, and remove `<script>` elements, whose flight payload the body dump otherwise carries. With both, the same-SHA repeat read identical on count, innerText and outerHTML on all four routes. That is the control that has to pass before any base-vs-branch zero is believed. Data drift is a separate problem (relative ages, odds ticks), so HO 718 diffed against a base build running at the same time on `:3007`, not against the morning's baseline.

---

## The assertion written to catch `closest()` was not the one that caught it (HO 719, Sep 2026)

HO 719's STEP 0 ruling added a fourth fixture reading to test property 3: three over-threshold rows inside the exempt `data-col-flow` container. The ruling said a `closest()` exemption "reads M1a 0 here and fails". HO 719 swapped `matches` for `closest` in the audit, ran the legs, and restored the file. **M1c-INNER passed.** The inner rows inherit the marking through `closest()`, but they do not compute multi-column, so the other half of the rule (marking **and** computed style) kept them scored. They landed on the dormant leak line instead, and **M1c-DORMANT failed, reading `marked, not col-flow 4`** (the three inner rows plus the dormant case). A second swap, `closest()` with the computed-style check removed as well, made M1c-INNER read `inner M1a 0` and fail, with M1c-POS at `M1c 5`.

So the two shapes of the defect are caught by different assertions. `closest()` plus the style check is caught by DORMANT; `closest()` alone is caught by INNER. The fixture catches both, but not through the assertion the ruling named. **An assertion's stated purpose is a prediction about which mutation it kills, and the only way to know is to apply the mutation.** Reasoning about it gets the conjunction wrong: the second half of an AND rule does work that is easy to credit to the first.

**A smaller trap from the same HO's DOM floor:** React renders a bare `data-col-flow` in JSX as `data-col-flow="true"`, not `=""`. The first check for "outerHTML differs by exactly the two attributes" searched for `data-col-flow=""`, found none in a branch that was 42 characters longer, and reported a mismatch. Searching for `="true"` (21 characters × 2) read exact. The same holds for every `data-viz-row` site, so a script that greps served HTML for a boolean marking must use `="true"`, or match the attribute name alone.
...
## `.next/cache/fetch-cache/` survives a build, a regeneration moves the mtime and not the bytes, and there is no `tags-manifest.json` to read a flush from (HO 726, Sep 2026)

**The local Data Cache outlives `npm run build`.** `fetch-cache/` held **283** files before and after HO 726's STEP 0 build, some dated **2026-09-13**, left by earlier sessions' local runs. On a fresh `next start`, a GET therefore rewrites any entry past its one-hour TTL with no flush at all. Row 4's two GETs rewrote one `meetings` entry and two `votes` entries before any POST had been made. This is the local sibling of the prod fact SKILL already records (*"The Data Cache survives deploys (oddities HO 312)"*, the HO 555 value-change convergence bullet); nothing in the tree recorded the local one. **A rewrite after a flush means the flush only if a control GET with no POST, run just before it, rewrote zero.**

**An entry's age is its file mtime, and its bytes carry no timestamp.** An `unstable_cache` entry is a FETCH record, `{"kind":"FETCH","data":{…},"revalidate":3600,"tags":["meetings"]}`. `FileSystemCache.get` takes `lastModified` from `fs.stat(...).mtime` (`node_modules/next/dist/server/lib/incremental-cache/file-system-cache.js:114`). A regeneration over unchanged data moves the mtime and leaves the file byte-identical: every rewrite HO 726 measured, TTL-driven or flush-driven, kept its sha256. **"The entry file's contents changed" cannot be read as regeneration.** Read the mtime, and print the hash to show it did not move. A moved hash is a data change during the measurement.

**At `next@16.3.4` a tag flush writes no file.** `FileSystemCache.revalidateTag` (`:43`) stores `{ stale, expired }` in `tagsManifest`, an in-memory `Map` (`tags-manifest.external.js:27`). `areTagsExpired` (`:28`) reads it on the next `get`. Nothing writes `tags-manifest.json`. HO 726's handoff prescribed reading that file as the flush's evidence, and it never comes into being. This is HO 683's class (method § Gates): an evidence path that is not there when it is read. At HO 683 the runner deleted it; here it is never written. **What to read instead:** start `next start` with `NEXT_PRIVATE_DEBUG_CACHE=1`, which logs `FileSystemCache: revalidateTag [ '<tag>' ] { expire: 0 }` at the Map's only write site. It changes logging, not behaviour. Pair it with the control GET above.

---

## In a CSS-module page, `[class*="__lead"]` matches `__leadwrap` first, and the reading comes back plausible (HO 727, Sep 2026)

**CSS-module class names are `<file>-module__<hash>__<local>`, so a substring selector on the local name also matches every local name that starts with it.** HO 727's close measured `.lead`'s right edge on `/welcome` with `document.querySelector('[class*="__lead"]')`. `querySelector` returns the first match in document order, which is the grid-area wrapper `.leadwrap` (`landing.module.css:60`), not the `<p class=lead>` inside it. The wrapper spans its banner column, so the "lead right edge" read **1438.9 at 2560 and 1071.2 at 1920**, where the paragraph's is **469.6** (its `max-width:46ch`). Nothing errored. The number was a real edge of a real element on the right page, and it would have produced a lead-to-stats gap of 48px instead of 1017.3.

**What caught it** was a second reading taken in the same pass, the right edge of the element's text ink (a `Range` over its contents), which disagreed with the box by ~970px. A box can legitimately be wider than its ink, so the disagreement alone proves nothing. Listing every match of the selector (`querySelectorAll`, with `className` and `tagName`) settled it.

**The rule for probes on module-scoped pages:** match the whole local name, not a prefix. Either anchor the end (`[class$="__lead"]`, safe only when the element carries one class) or test the class list (`[...el.classList].some(c => c.endsWith("__lead"))`). And when a probe names a class that is a prefix of another in the same module (`lead`/`leadwrap`, `stat`/`stats`), list all its matches once before trusting the first. The HO 727 STEP 0 geometry did not hit this: its keys (`ctagrid`, `stats`, `flourish`, `btnPrimary`, `btnSecondary`, `micro1`, `micro2`) match first on the intended element, as the per-element rects confirm (`.stats` precedes its `.stat` cells, and `__stats` does not match `__stat`).

---

## `/welcome`'s "every font-family is a token" was false from the commit that wrote it, and the control that looked like it certified the claim had measured the file that commit replaced (HO 727, Sep 2026)

**Readings.** `app/welcome/landing.module.css` holds **7** `font-family` declarations at `72af9a5`, **1 off-token**: the flourish's declared fallback stack `'Segoe UI Symbol','Apple Symbols','Noto Sans Symbols 2',serif` at `:126`. At `8f0613b` it holds **6**, **0 off-token**. Two standing sentences were therefore **false at `main` and are true as of HO 727**, as a side effect of removing the flourish and not by intent: the module's header comment (*"Every font-family below is var(--font-mono) or var(--sans)"*) and SKILL `:1847` (*"The type is token-consuming: `var(--font-mono)` / `var(--sans)` throughout"*).

**The control on record.** `docs/backlog.md:180` (filed in `b284a42`, 2026-08-18) says that at `4ae76a8` (2026-08-17) *"all 10 of its `font-family` declarations were already on-token … with a control grep for a non-token declaration returning zero lines."* That reading is correct. `git show 4ae76a8:app/welcome/landing.module.css` has 10 declarations, 0 off-token.

**The provenance, measured with `git log -S"Segoe UI Symbol" -- app/welcome/landing.module.css` and not assumed.** The architect's first reading was that the stack *"arrived with the HO 697–699 font work, after the control, and nobody re-ran it."* The pickaxe does not support that. Scoped to the module, the stack has exactly two touching commits: **`b5a9455` (HO 670's rebuild, 2026-08-18 09:24)**, which added it, and `6bd6749` (HO 727), which removed it. **Unscoped, the same pickaxe returns four, and the extra two are not the module.** `b284a42` (HO 670 docs) committed `docs/design/welcome-formats-mock.html`, whose `:82` `body.flourish-fallback .flourish` rule carries the same stack. HO 727's own `docs` commit adds the string by quoting it in this entry and the roadmap block. Run the command with the path scope as written. HO 697, 698 and 699 edited the file but never this declaration. **The same commit `b5a9455` also wrote the header comment**: `git show b5a9455:app/welcome/landing.module.css` has the claim at `:6` and the off-token stack at `:92`, 7 declarations with 1 off-token. HO 670's own handoff required that stack (`670-welcome-board-rebuild.md:58`, *"The flourish is a rendering hazard and must be handled, not pasted"*). The SKILL sentence landed 15 minutes later in `4d34e5c`. **Both claims were false at the moment they were written.** The file `4ae76a8`'s control measured was the **HO 361 module**, whose header reads *"HO 361 — landing page, ported from … landing.html"*, and `b5a9455` replaced it wholesale.

**The generalisation, stated once.** A control certifies the file it ran on. Replacing the file retires the control, however much the new file resembles the old one in name, path or intent. Here the claim was carried across the replacement by the same session that wrote the replacement, so the zero-lines grep looked as if it covered a module it never read. It is the file-level form of the roadmap rule *"a measurement is about the table it was taken on"* (HO 677) and of method § Gates' zero that cannot become non-zero. **The check that would have caught it** is the one the claim itself names, re-run on the commit that wrote the claim: `grep font-family <file> | grep -v 'var(--font-mono)\|var(--sans)'`, which returns one line at `b5a9455`. Nothing is left to close; the claims are true now.

---

## The layout audit records a trailing gap it never scores, by design, and a handoff built a gate on it (HO 728, Sep 2026)

**What the audit does.** `scripts/diagnostic/layout-audit-606.ts` writes `trailingGapPx = innerRight − rightMost` into every M1 row (`:716`, `:722`) and into the per-route artifact JSON. **No count reads it.** Every threshold test in the script is on an interior gap: the candidate-leak line (`:708`, the local `interior`), the per-route counts `overThreshold` / `m1aCount` / `m1bCount` (`:1116-1118`), the negative-control legs (`:1266`, `:1274`), the M1c leak and inner lines (`:1397`, `:1406`, `:1418`), and the leg summaries (`:1582`, `:1595`). The site total at `:1983`, printed as *"M1 rows over 120px"*, is the sum of `m1aCount` and `m1bCount`, so it too is built from interior gaps alone.

**It is deliberate, and the file says so.** The header (`:9-12`): *"Reports largest INTERIOR gap and, separately, the TRAILING gap. The trailing gap is measured and is NOT a defect — it is the target state. Reporting both means 'M1 = 0' cannot be reached by deleting rows."* Under C1 a row packed to the left, with its slack at the right end, is what remediation aims for. The trailing figure exists so a row cannot pass by disappearing, not as a second defect measure.

**Worked examples from one `/welcome` run** (`--route welcome`, 2560, hit 2, `f30b597`). **BREAKING: trailing 1809, interior 14, count 0.** If trailing counted, it would be the largest hit on the page. **The panel `rmeta` rows: trailing 428–632, interior 9, count 0**, dozens of them. The page's two counted rows are the rail (interior 1857, trailing 0) and the bottom bar (interior 1593, trailing 0), the opposite shape.
...
## `innerText` applies `text-transform`, so a case-sensitive count of a styled label reads the same zero as an unrendered page (HO 736, Sep 2026)

The HO 736 read-back had to confirm that fifteen roster rows now render **Advanced** where they used to render *Won primary*. The reading instrument counted `document.body.innerText` against `new RegExp('Advanced', 'g')` and reported **`advanced: 0` on all four captures** — both widths, both races — while the same pages, fetched with `curl`, carried four and two literal `Advanced` strings in the served HTML.

**`innerText` is the rendered text, and rendering includes `text-transform`.** CBT styles that badge uppercase in CSS; the DOM text node is `Advanced` and `innerText` hands back `ADVANCED`. `textContent` would have returned `Advanced`, and the served HTML does — which is why the curl and the browser disagreed about a page neither of them was wrong about.

**The reason it belongs here rather than in a style guide is the shape of the failure, not the API.** The probe also counted `wonPrimary` and `primaryWinner` as the controls, and those were *correctly* zero. So the reading was `{advanced: 0, wonPrimary: 0, primaryWinner: 0}` — and **that is exactly what a page which never rendered its roster would give**, which is exactly what a page behind a bot wall would give, and what a wrong selector would give. Every number in it was zero and every zero looked like evidence (§ Gates: *a gate you cannot fail is not a gate* — say what the check reads if the work was never done, and this read the same).
