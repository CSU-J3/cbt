...
> **As of HO 388 · 2026-06-30.** This block is the **source of truth**. Future updates edit this block, not memory or off-repo notes. *(Prose reconciled through the HO 382–386 stage-correctness arc; that arc + the HO 379 smoke harness are correctness/infra work that minted no new theme bars — see the "Also (HO 379, 382–386)" note below. The HO 364 brand rename and the HO 366/367 `/hearings` + `/members` header consolidations are within-theme polish — Home / Foundation / Member depth — also accounted-for here, not deferred. The Foundation and Member-depth baselines remain off-repo last-known values, still owed a re-derivation at the next full audit.)*
