...
> **As of HO 403 · 2026-07-02.** This block is the **source of truth**. Future updates edit this block, not memory or off-repo notes. *(This sweep folds the HO 402 member ID crosswalk — see the "Also (HO 402)" note below. **Not yet folded:** HO 398's race→news surface (shipped `5f26ee9`) — it graduated the banked race→news candidate but hasn't been reconciled into a theme bar here; owed at the next full audit. Prose reconciled through the HO 382–386 stage-correctness arc; that arc + the HO 379 smoke harness are correctness/infra work that minted no new theme bars — see the "Also (HO 379, 382–386)" note below. The HO 364 brand rename and the HO 366/367 `/hearings` + `/members` header consolidations are within-theme polish — Home / Foundation / Member depth — also accounted-for here, not deferred. The Foundation and Member-depth baselines remain off-repo last-known values, still owed a re-derivation at the next full audit.)*
