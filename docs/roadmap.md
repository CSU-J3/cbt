...
> **As of HO 377 · 2026-06-26.** This block is the **source of truth**. Future updates edit this block, not memory or off-repo notes. *(Prose reconciled through the B5 arc; the HO 364 brand rename and the HO 366/367 `/hearings` + `/members` header consolidations are within-theme polish — Home / Foundation / Member depth — that minted no new theme bars, so they're accounted-for here, not deferred. The Foundation and Member-depth baselines remain off-repo last-known values, still owed a re-derivation at the next full audit.)*
