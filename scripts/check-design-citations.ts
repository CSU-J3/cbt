 ...
 * a mock NAMED OUTSIDE THE CONVENTION and cited by bare name
 * (`ideology-cut1-members-insitu.html`, `dashboard-hill-fit.html`,
 * `dashboard-layout-target.html`, …). G2 recognises `mock-<HO>-<slug>.html` only.
 * That is not a date boundary — the name has been in use since HO 507 and plenty
 * of live mocks do not follow it — which is exactly why the tracked-by-default
 * rule, not the grammar, is what makes a citation resolve.
 *
 * This file is NOT exempt from its own scan, and neither is either README. They
 * simply never write a concrete path below the disposable directory — they say
 * so in words. (HO 695's postscript, wearing a costume: the cheapest defence
 * against an escape is not needing one.) The ONE exemption is the allowlist
 * itself, and it is structural rather than convenient — see the note at the
 * skip.
 */
...
type Klass = "FORBIDDEN" | "MALFORMED" | "DANGLING" | "STALE" | "ORPHAN";
...
function main(): void {
  ...
  for (const file of tracked) {
    ...
    // THE ALLOWLIST IS A REGISTRY, NOT PROSE — its keys are the tokens it
    // excuses, so scanning it would make every entry cite itself and ORPHAN
    // could never fire. (It did not, on the first cut: a planted entry for a
    // token nothing references read OK, because the plant was in the file being
    // read. A class that cannot fire is not a class.) Skipping it also makes a
    // bogus `scratch/` entry self-enforcing: nothing else can match it, so it
    // reds as ORPHAN rather than sitting there looking authoritative.
  ...
  // An allowlist entry nothing cites is a hole waiting for that name to be used
  // again — the odds gate's second direction in this gate's terms. Removing a
  // citation without removing its entry leaves the next author silently covered.
  for (const token of allow.keys()) {
    if (allowlistHit.has(token)) continue;
    problems.push({ file: ALLOWLIST_PATH, line: 0, token, klass: "ORPHAN" });
  }
